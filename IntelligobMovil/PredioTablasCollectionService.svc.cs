...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// PredioTablasCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class PredioTablasCollectionService : IPredioTablasCollectionService
		{
			...
			#region IPredioTablasCollectionService Members
			...
			{
				return this.HandleReadPredioTabla(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<PredioTablaDto> ReadPredioTablas()
			{
				return this.HandleReadPredioTablas(itemUriTemplate);
			}
			...
			/// <returns></returns>
			public ItemInfo<PredioTablaDto> CreatePredioTabla(PredioTablaDto prediotabla)
			{
				return this.HandleCreatePredioTabla(prediotabla, itemUriTemplate);
			}
			...
			/// <returns></returns>
			public PredioTablaDto UpdatePredioTabla(string dtoKey, PredioTablaDto prediotabla)
			{
				return this.HandleUpdatePredioTabla(dtoKey, prediotabla);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeletePredioTabla(string dtoKey)
			{
				this.HandleDeletePredioTabla(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of PredioTablas returned per page.
			/// </summary>
			public int PredioTablasPerPageCount
			{
				get 
				{ 
					return this.prediotablasPerPageCount; 
				}
				set 
				{
					if (value != this.prediotablasPerPageCount)
					{
						this.prediotablasPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IPredioTablaService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new PredioTablaService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a prediotabla to the database and return the prediotabla with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="prediotabla">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected PredioTablaDto Create(PredioTablaDto prediotabla)
			{
				try
				{
					string dtoKey = this.Service.Add(prediotabla);
					this.UnitOfWork.SaveChanges();
					PredioTablaDto addedPredioTabla = this.Read(dtoKey);
					if (addedPredioTabla == null)
					{
			...
			/// <summary>
			/// Returns the PredioTablaDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected PredioTablaDto Read(string dtoKey)
			{
				try
				{
					PredioTablaDto prediotabla = this.Service.GetByKey(dtoKey);
					if (prediotabla == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, PredioTablaDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the prediotabla with the specified dto key in the database. 
			...
			/// <param name="prediotabla">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, PredioTablaDto prediotabla)
			{
				try
				{
					if (dtoKey != prediotabla.DtoKey && dtoKey != prediotabla.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(prediotabla);
			...
			/// <summary>
			/// Deletes the prediotabla with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					PredioTablaDto prediotabla = Read(dtoKey);
					if (prediotabla == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(prediotabla);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Handles adding of prediotabla with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="prediotabla">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<PredioTablaDto> HandleCreatePredioTabla(PredioTablaDto prediotabla, UriTemplate template)
			{
				PredioTablaDto addedPredioTabla = this.Create(prediotabla);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedPredioTabla.DtoKey, template));
				ItemInfo<PredioTablaDto> itemInfo = new ItemInfo<PredioTablaDto>();
				itemInfo.Item = addedPredioTabla;
				itemInfo.EditLink = GetItemLink(addedPredioTabla.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of prediotabla with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected PredioTablaDto HandleReadPredioTabla(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of PredioTablas with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<PredioTablaDto> HandleReadPredioTablas(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.PredioTablasPerPageCount > maxEntities)
					{
						this.PredioTablasPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.PredioTablasPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.PredioTablasPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, PredioTablaDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.PredioTablasPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, PredioTablaDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<PredioTablaDto>> itemsInfo = new List<ItemInfo<PredioTablaDto>>();
	
					foreach (KeyValuePair<string, PredioTablaDto> item in items)
					{
						ItemInfo<PredioTablaDto> itemInfo = new ItemInfo<PredioTablaDto>();
					...
					return new ItemInfoList<PredioTablaDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of prediotabla with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="prediotabla">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected PredioTablaDto HandleUpdatePredioTabla(string dtoKey, PredioTablaDto prediotabla)
			{
				this.Update(dtoKey, prediotabla);
	
				return prediotabla;
			}
	
			/// <summary>
			/// Handles deleting of prediotabla with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeletePredioTabla(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// PredioTablasCollectionService finalizer.
			/// </summary>
			~PredioTablasCollectionService()
			{
