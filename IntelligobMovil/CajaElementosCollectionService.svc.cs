...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// CajaElementosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class CajaElementosCollectionService : ICajaElementosCollectionService
		{
			...
			#region ICajaElementosCollectionService Members
			...
			{
				return this.HandleReadCajaElemento(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<CajaElementoDto> ReadCajaElementos()
			{
				return this.HandleReadCajaElementos(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="cajaelemento">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<CajaElementoDto> CreateCajaElemento(CajaElementoDto cajaelemento)
			{
				return this.HandleCreateCajaElemento(cajaelemento, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="cajaelemento">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public CajaElementoDto UpdateCajaElemento(string dtoKey, CajaElementoDto cajaelemento)
			{
				return this.HandleUpdateCajaElemento(dtoKey, cajaelemento);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteCajaElemento(string dtoKey)
			{
				this.HandleDeleteCajaElemento(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of CajaElementos returned per page.
			/// </summary>
			public int CajaElementosPerPageCount
			{
				get 
				{ 
					return this.cajaelementosPerPageCount; 
				}
				set 
				{
					if (value != this.cajaelementosPerPageCount)
					{
						this.cajaelementosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual ICajaElementoService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new CajaElementoService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a cajaelemento to the database and return the cajaelemento with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="cajaelemento">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected CajaElementoDto Create(CajaElementoDto cajaelemento)
			{
				try
				{
					string dtoKey = this.Service.Add(cajaelemento);
					this.UnitOfWork.SaveChanges();
					CajaElementoDto addedCajaElemento = this.Read(dtoKey);
					if (addedCajaElemento == null)
					{
			...
			/// <summary>
			/// Returns the CajaElementoDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected CajaElementoDto Read(string dtoKey)
			{
				try
				{
					CajaElementoDto cajaelemento = this.Service.GetByKey(dtoKey);
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, CajaElementoDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the cajaelemento with the specified dto key in the database. 
			...
			/// <param name="cajaelemento">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, CajaElementoDto cajaelemento)
			{
				try
				{
					if (dtoKey != cajaelemento.DtoKey && dtoKey != cajaelemento.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(cajaelemento);
			...
			/// <summary>
			/// Deletes the cajaelemento with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					CajaElementoDto cajaelemento = Read(dtoKey);
					if (cajaelemento == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(cajaelemento);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Returns the cajaelemento count.
			/// </summary>
			/// <returns></returns>
			protected int Count()
			{
			...
			/// <summary>
			/// Handles adding of cajaelemento with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="cajaelemento">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<CajaElementoDto> HandleCreateCajaElemento(CajaElementoDto cajaelemento, UriTemplate template)
			{
				CajaElementoDto addedCajaElemento = this.Create(cajaelemento);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedCajaElemento.DtoKey, template));
				ItemInfo<CajaElementoDto> itemInfo = new ItemInfo<CajaElementoDto>();
				itemInfo.Item = addedCajaElemento;
				itemInfo.EditLink = GetItemLink(addedCajaElemento.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of cajaelemento with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected CajaElementoDto HandleReadCajaElemento(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of CajaElementos with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<CajaElementoDto> HandleReadCajaElementos(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.CajaElementosPerPageCount > maxEntities)
					{
						this.CajaElementosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.CajaElementosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.CajaElementosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, CajaElementoDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.CajaElementosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, CajaElementoDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<CajaElementoDto>> itemsInfo = new List<ItemInfo<CajaElementoDto>>();
	
					foreach (KeyValuePair<string, CajaElementoDto> item in items)
					{
						ItemInfo<CajaElementoDto> itemInfo = new ItemInfo<CajaElementoDto>();
					...
					return new ItemInfoList<CajaElementoDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of cajaelemento with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="cajaelemento">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected CajaElementoDto HandleUpdateCajaElemento(string dtoKey, CajaElementoDto cajaelemento)
			{
				this.Update(dtoKey, cajaelemento);
	
				return cajaelemento;
			}
	
			/// <summary>
			/// Handles deleting of cajaelemento with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteCajaElemento(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// CajaElementosCollectionService finalizer.
			/// </summary>
			~CajaElementosCollectionService()
			{
