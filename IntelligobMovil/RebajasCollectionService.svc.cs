...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// RebajasCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class RebajasCollectionService : IRebajasCollectionService
		{
			...
			#region IRebajasCollectionService Members
			...
			{
				return this.HandleReadRebaja(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<RebajaDto> ReadRebajas()
			{
				return this.HandleReadRebajas(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="rebaja">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<RebajaDto> CreateRebaja(RebajaDto rebaja)
			{
				return this.HandleCreateRebaja(rebaja, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="rebaja">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public RebajaDto UpdateRebaja(string dtoKey, RebajaDto rebaja)
			{
				return this.HandleUpdateRebaja(dtoKey, rebaja);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteRebaja(string dtoKey)
			{
				this.HandleDeleteRebaja(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Rebajas returned per page.
			/// </summary>
			public int RebajasPerPageCount
			{
				get 
				{ 
					return this.rebajasPerPageCount; 
				}
				set 
				{
					if (value != this.rebajasPerPageCount)
					{
						this.rebajasPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IRebajaService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new RebajaService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a rebaja to the database and return the rebaja with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="rebaja">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected RebajaDto Create(RebajaDto rebaja)
			{
				try
				{
					string dtoKey = this.Service.Add(rebaja);
					this.UnitOfWork.SaveChanges();
					RebajaDto addedRebaja = this.Read(dtoKey);
					if (addedRebaja == null)
					{
			...
			/// <summary>
			/// Returns the RebajaDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected RebajaDto Read(string dtoKey)
			{
				try
				{
					RebajaDto rebaja = this.Service.GetByKey(dtoKey);
					if (rebaja == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, RebajaDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the rebaja with the specified dto key in the database. 
			...
			/// <param name="rebaja">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, RebajaDto rebaja)
			{
				try
				{
					if (dtoKey != rebaja.DtoKey && dtoKey != rebaja.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
			...
			/// <summary>
			/// Deletes the rebaja with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			/// <summary>
			/// Handles adding of rebaja with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="rebaja">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<RebajaDto> HandleCreateRebaja(RebajaDto rebaja, UriTemplate template)
			{
				RebajaDto addedRebaja = this.Create(rebaja);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedRebaja.DtoKey, template));
				ItemInfo<RebajaDto> itemInfo = new ItemInfo<RebajaDto>();
				itemInfo.Item = addedRebaja;
				itemInfo.EditLink = GetItemLink(addedRebaja.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of rebaja with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected RebajaDto HandleReadRebaja(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Rebajas with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<RebajaDto> HandleReadRebajas(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.RebajasPerPageCount > maxEntities)
					{
						this.RebajasPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.RebajasPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.RebajasPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, RebajaDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.RebajasPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, RebajaDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<RebajaDto>> itemsInfo = new List<ItemInfo<RebajaDto>>();
	
					foreach (KeyValuePair<string, RebajaDto> item in items)
					{
						ItemInfo<RebajaDto> itemInfo = new ItemInfo<RebajaDto>();
					...
					return new ItemInfoList<RebajaDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of rebaja with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="rebaja">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected RebajaDto HandleUpdateRebaja(string dtoKey, RebajaDto rebaja)
			{
				this.Update(dtoKey, rebaja);
	
				return rebaja;
			}
	
			/// <summary>
			/// Handles deleting of rebaja with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteRebaja(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// RebajasCollectionService finalizer.
			/// </summary>
			~RebajasCollectionService()
			{
