...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// CuentaBancariasCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class CuentaBancariasCollectionService : ICuentaBancariasCollectionService
		{
			...
			/// </summary>
			private int cuentabancariasPerPageCount = 5;
			...
			#region ICuentaBancariasCollectionService Members
			...
			{
				return this.HandleReadCuentaBancaria(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<CuentaBancariaDto> ReadCuentaBancarias()
			{
				return this.HandleReadCuentaBancarias(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="cuentabancaria">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<CuentaBancariaDto> CreateCuentaBancaria(CuentaBancariaDto cuentabancaria)
			{
				return this.HandleCreateCuentaBancaria(cuentabancaria, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="cuentabancaria">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public CuentaBancariaDto UpdateCuentaBancaria(string dtoKey, CuentaBancariaDto cuentabancaria)
			{
				return this.HandleUpdateCuentaBancaria(dtoKey, cuentabancaria);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteCuentaBancaria(string dtoKey)
			{
				this.HandleDeleteCuentaBancaria(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of CuentaBancarias returned per page.
			/// </summary>
			public int CuentaBancariasPerPageCount
			{
				get 
				{ 
					return this.cuentabancariasPerPageCount; 
				}
				set 
				{
					if (value != this.cuentabancariasPerPageCount)
					{
						this.cuentabancariasPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual ICuentaBancariaService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new CuentaBancariaService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a cuentabancaria to the database and return the cuentabancaria with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="cuentabancaria">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected CuentaBancariaDto Create(CuentaBancariaDto cuentabancaria)
			{
				try
				{
					string dtoKey = this.Service.Add(cuentabancaria);
					this.UnitOfWork.SaveChanges();
					CuentaBancariaDto addedCuentaBancaria = this.Read(dtoKey);
					if (addedCuentaBancaria == null)
					{
			...
			/// <summary>
			/// Returns the CuentaBancariaDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected CuentaBancariaDto Read(string dtoKey)
			{
				try
				{
					CuentaBancariaDto cuentabancaria = this.Service.GetByKey(dtoKey);
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, CuentaBancariaDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the cuentabancaria with the specified dto key in the database. 
			...
			/// <param name="cuentabancaria">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, CuentaBancariaDto cuentabancaria)
			{
				try
				{
					if (dtoKey != cuentabancaria.DtoKey && dtoKey != cuentabancaria.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(cuentabancaria);
			...
			/// <summary>
			/// Deletes the cuentabancaria with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					CuentaBancariaDto cuentabancaria = Read(dtoKey);
					if (cuentabancaria == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(cuentabancaria);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Returns the cuentabancaria count.
			/// </summary>
			/// <returns></returns>
			protected int Count()
			{
			...
			/// <summary>
			/// Handles adding of cuentabancaria with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="cuentabancaria">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<CuentaBancariaDto> HandleCreateCuentaBancaria(CuentaBancariaDto cuentabancaria, UriTemplate template)
			{
				CuentaBancariaDto addedCuentaBancaria = this.Create(cuentabancaria);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedCuentaBancaria.DtoKey, template));
				ItemInfo<CuentaBancariaDto> itemInfo = new ItemInfo<CuentaBancariaDto>();
				itemInfo.Item = addedCuentaBancaria;
				itemInfo.EditLink = GetItemLink(addedCuentaBancaria.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of cuentabancaria with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected CuentaBancariaDto HandleReadCuentaBancaria(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of CuentaBancarias with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<CuentaBancariaDto> HandleReadCuentaBancarias(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.CuentaBancariasPerPageCount > maxEntities)
					{
						this.CuentaBancariasPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.CuentaBancariasPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.CuentaBancariasPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, CuentaBancariaDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.CuentaBancariasPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, CuentaBancariaDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<CuentaBancariaDto>> itemsInfo = new List<ItemInfo<CuentaBancariaDto>>();
	
					foreach (KeyValuePair<string, CuentaBancariaDto> item in items)
					{
						ItemInfo<CuentaBancariaDto> itemInfo = new ItemInfo<CuentaBancariaDto>();
					...
					return new ItemInfoList<CuentaBancariaDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of cuentabancaria with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="cuentabancaria">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected CuentaBancariaDto HandleUpdateCuentaBancaria(string dtoKey, CuentaBancariaDto cuentabancaria)
			{
				this.Update(dtoKey, cuentabancaria);
	
				return cuentabancaria;
			}
	
			/// <summary>
			/// Handles deleting of cuentabancaria with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteCuentaBancaria(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// CuentaBancariasCollectionService finalizer.
			/// </summary>
			~CuentaBancariasCollectionService()
			{
