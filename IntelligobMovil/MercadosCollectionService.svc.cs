...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// MercadosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class MercadosCollectionService : IMercadosCollectionService
		{
			...
			/// </summary>
			private int mercadosPerPageCount = 5;
			...
			#region IMercadosCollectionService Members
			...
			{
				return this.HandleReadMercado(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<MercadoDto> ReadMercados()
			{
				return this.HandleReadMercados(itemUriTemplate);
			}
			...
			/// <returns></returns>
			public ItemInfo<MercadoDto> CreateMercado(MercadoDto mercado)
			{
				return this.HandleCreateMercado(mercado, itemUriTemplate);
			}
			...
			/// <returns></returns>
			public MercadoDto UpdateMercado(string dtoKey, MercadoDto mercado)
			{
				return this.HandleUpdateMercado(dtoKey, mercado);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteMercado(string dtoKey)
			{
				this.HandleDeleteMercado(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Mercados returned per page.
			/// </summary>
			public int MercadosPerPageCount
			{
				get 
				{ 
					return this.mercadosPerPageCount; 
				}
				set 
				{
					if (value != this.mercadosPerPageCount)
					{
						this.mercadosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IMercadoService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new MercadoService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a mercado to the database and return the mercado with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="mercado">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected MercadoDto Create(MercadoDto mercado)
			{
				try
				{
					string dtoKey = this.Service.Add(mercado);
					this.UnitOfWork.SaveChanges();
					MercadoDto addedMercado = this.Read(dtoKey);
					if (addedMercado == null)
					{
			...
			/// <summary>
			/// Returns the MercadoDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected MercadoDto Read(string dtoKey)
			{
				try
				{
					MercadoDto mercado = this.Service.GetByKey(dtoKey);
					if (mercado == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, MercadoDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the mercado with the specified dto key in the database. 
			...
			/// <param name="mercado">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, MercadoDto mercado)
			{
				try
				{
					if (dtoKey != mercado.DtoKey && dtoKey != mercado.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
			...
			/// <summary>
			/// Deletes the mercado with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					MercadoDto mercado = Read(dtoKey);
					if (mercado == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
			...
			/// <summary>
			/// Handles adding of mercado with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="mercado">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<MercadoDto> HandleCreateMercado(MercadoDto mercado, UriTemplate template)
			{
				MercadoDto addedMercado = this.Create(mercado);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedMercado.DtoKey, template));
				ItemInfo<MercadoDto> itemInfo = new ItemInfo<MercadoDto>();
				itemInfo.Item = addedMercado;
				itemInfo.EditLink = GetItemLink(addedMercado.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of mercado with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected MercadoDto HandleReadMercado(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Mercados with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<MercadoDto> HandleReadMercados(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.MercadosPerPageCount > maxEntities)
					{
						this.MercadosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.MercadosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.MercadosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, MercadoDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.MercadosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, MercadoDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<MercadoDto>> itemsInfo = new List<ItemInfo<MercadoDto>>();
	
					foreach (KeyValuePair<string, MercadoDto> item in items)
					{
						ItemInfo<MercadoDto> itemInfo = new ItemInfo<MercadoDto>();
					...
					return new ItemInfoList<MercadoDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of mercado with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="mercado">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected MercadoDto HandleUpdateMercado(string dtoKey, MercadoDto mercado)
			{
				this.Update(dtoKey, mercado);
	
				return mercado;
			}
	
			/// <summary>
			/// Handles deleting of mercado with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteMercado(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// MercadosCollectionService finalizer.
			/// </summary>
			~MercadosCollectionService()
			{
