...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// ConceptosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class ConceptosCollectionService : IConceptosCollectionService
		{
			...
			/// </summary>
			private int conceptosPerPageCount = 5;
			...
			#region IConceptosCollectionService Members
			...
			{
				return this.HandleReadConcepto(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<ConceptoDto> ReadConceptos()
			{
				return this.HandleReadConceptos(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="concepto">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<ConceptoDto> CreateConcepto(ConceptoDto concepto)
			{
				return this.HandleCreateConcepto(concepto, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="concepto">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public ConceptoDto UpdateConcepto(string dtoKey, ConceptoDto concepto)
			{
				return this.HandleUpdateConcepto(dtoKey, concepto);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteConcepto(string dtoKey)
			{
				this.HandleDeleteConcepto(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Conceptos returned per page.
			/// </summary>
			public int ConceptosPerPageCount
			{
				get 
				{ 
					return this.conceptosPerPageCount; 
				}
				set 
				{
					if (value != this.conceptosPerPageCount)
					{
						this.conceptosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IConceptoService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new ConceptoService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a concepto to the database and return the concepto with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="concepto">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected ConceptoDto Create(ConceptoDto concepto)
			{
				try
				{
					string dtoKey = this.Service.Add(concepto);
					this.UnitOfWork.SaveChanges();
					ConceptoDto addedConcepto = this.Read(dtoKey);
					if (addedConcepto == null)
					{
			...
			/// <summary>
			/// Returns the ConceptoDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ConceptoDto Read(string dtoKey)
			{
				try
				{
					ConceptoDto concepto = this.Service.GetByKey(dtoKey);
					if (concepto == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, ConceptoDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the concepto with the specified dto key in the database. 
			...
			/// <param name="concepto">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, ConceptoDto concepto)
			{
				try
				{
					if (dtoKey != concepto.DtoKey && dtoKey != concepto.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
			...
			/// <summary>
			/// Deletes the concepto with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					ConceptoDto concepto = Read(dtoKey);
					if (concepto == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
			...
			/// <summary>
			/// Handles adding of concepto with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="concepto">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<ConceptoDto> HandleCreateConcepto(ConceptoDto concepto, UriTemplate template)
			{
				ConceptoDto addedConcepto = this.Create(concepto);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedConcepto.DtoKey, template));
				ItemInfo<ConceptoDto> itemInfo = new ItemInfo<ConceptoDto>();
				itemInfo.Item = addedConcepto;
				itemInfo.EditLink = GetItemLink(addedConcepto.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of concepto with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ConceptoDto HandleReadConcepto(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Conceptos with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<ConceptoDto> HandleReadConceptos(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.ConceptosPerPageCount > maxEntities)
					{
						this.ConceptosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.ConceptosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.ConceptosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, ConceptoDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.ConceptosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, ConceptoDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<ConceptoDto>> itemsInfo = new List<ItemInfo<ConceptoDto>>();
	
					foreach (KeyValuePair<string, ConceptoDto> item in items)
					{
						ItemInfo<ConceptoDto> itemInfo = new ItemInfo<ConceptoDto>();
					...
					return new ItemInfoList<ConceptoDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of concepto with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="concepto">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected ConceptoDto HandleUpdateConcepto(string dtoKey, ConceptoDto concepto)
			{
				this.Update(dtoKey, concepto);
	
				return concepto;
			}
	
			/// <summary>
			/// Handles deleting of concepto with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteConcepto(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// ConceptosCollectionService finalizer.
			/// </summary>
			~ConceptosCollectionService()
			{
