...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// ReportesCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class ReportesCollectionService : IReportesCollectionService
		{
			...
			/// </summary>
			private int reportesPerPageCount = 5;
			...
			#region IReportesCollectionService Members
			...
			{
				return this.HandleReadReporte(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<ReporteDto> ReadReportes()
			{
				return this.HandleReadReportes(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="reporte">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<ReporteDto> CreateReporte(ReporteDto reporte)
			{
				return this.HandleCreateReporte(reporte, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="reporte">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public ReporteDto UpdateReporte(string dtoKey, ReporteDto reporte)
			{
				return this.HandleUpdateReporte(dtoKey, reporte);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteReporte(string dtoKey)
			{
				this.HandleDeleteReporte(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Reportes returned per page.
			/// </summary>
			public int ReportesPerPageCount
			{
				get 
				{ 
					return this.reportesPerPageCount; 
				}
				set 
				{
					if (value != this.reportesPerPageCount)
					{
						this.reportesPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IReporteService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new ReporteService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a reporte to the database and return the reporte with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="reporte">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected ReporteDto Create(ReporteDto reporte)
			{
				try
				{
					string dtoKey = this.Service.Add(reporte);
					this.UnitOfWork.SaveChanges();
					ReporteDto addedReporte = this.Read(dtoKey);
					if (addedReporte == null)
					{
			...
			/// <summary>
			/// Returns the ReporteDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ReporteDto Read(string dtoKey)
			{
				try
				{
					ReporteDto reporte = this.Service.GetByKey(dtoKey);
					if (reporte == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, ReporteDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the reporte with the specified dto key in the database. 
			...
			/// <param name="reporte">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, ReporteDto reporte)
			{
				try
				{
					if (dtoKey != reporte.DtoKey && dtoKey != reporte.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
			...
			/// <summary>
			/// Deletes the reporte with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					ReporteDto reporte = Read(dtoKey);
					if (reporte == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
			...
			/// <summary>
			/// Handles adding of reporte with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="reporte">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<ReporteDto> HandleCreateReporte(ReporteDto reporte, UriTemplate template)
			{
				ReporteDto addedReporte = this.Create(reporte);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedReporte.DtoKey, template));
				ItemInfo<ReporteDto> itemInfo = new ItemInfo<ReporteDto>();
				itemInfo.Item = addedReporte;
				itemInfo.EditLink = GetItemLink(addedReporte.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of reporte with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ReporteDto HandleReadReporte(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Reportes with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<ReporteDto> HandleReadReportes(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.ReportesPerPageCount > maxEntities)
					{
						this.ReportesPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.ReportesPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.ReportesPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, ReporteDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.ReportesPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, ReporteDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<ReporteDto>> itemsInfo = new List<ItemInfo<ReporteDto>>();
	
					foreach (KeyValuePair<string, ReporteDto> item in items)
					{
						ItemInfo<ReporteDto> itemInfo = new ItemInfo<ReporteDto>();
					...
					return new ItemInfoList<ReporteDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of reporte with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="reporte">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected ReporteDto HandleUpdateReporte(string dtoKey, ReporteDto reporte)
			{
				this.Update(dtoKey, reporte);
	
				return reporte;
			}
	
			/// <summary>
			/// Handles deleting of reporte with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteReporte(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// ReportesCollectionService finalizer.
			/// </summary>
			~ReportesCollectionService()
			{
