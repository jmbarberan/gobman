...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// RebajasRubrosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class RebajasRubrosCollectionService : IRebajasRubrosCollectionService
		{
			...
			#region IRebajasRubrosCollectionService Members
			...
			{
				return this.HandleReadRebajasRubro(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<RebajasRubroDto> ReadRebajasRubros()
			{
				return this.HandleReadRebajasRubros(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="rebajasrubro">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<RebajasRubroDto> CreateRebajasRubro(RebajasRubroDto rebajasrubro)
			{
				return this.HandleCreateRebajasRubro(rebajasrubro, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="rebajasrubro">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public RebajasRubroDto UpdateRebajasRubro(string dtoKey, RebajasRubroDto rebajasrubro)
			{
				return this.HandleUpdateRebajasRubro(dtoKey, rebajasrubro);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteRebajasRubro(string dtoKey)
			{
				this.HandleDeleteRebajasRubro(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of RebajasRubros returned per page.
			/// </summary>
			public int RebajasRubrosPerPageCount
			{
				get 
				{ 
					return this.rebajasrubrosPerPageCount; 
				}
				set 
				{
					if (value != this.rebajasrubrosPerPageCount)
					{
						this.rebajasrubrosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IRebajasRubroService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new RebajasRubroService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a rebajasrubro to the database and return the rebajasrubro with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="rebajasrubro">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected RebajasRubroDto Create(RebajasRubroDto rebajasrubro)
			{
				try
				{
					string dtoKey = this.Service.Add(rebajasrubro);
					this.UnitOfWork.SaveChanges();
					RebajasRubroDto addedRebajasRubro = this.Read(dtoKey);
					if (addedRebajasRubro == null)
					{
			...
			/// <summary>
			/// Returns the RebajasRubroDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected RebajasRubroDto Read(string dtoKey)
			{
				try
				{
					RebajasRubroDto rebajasrubro = this.Service.GetByKey(dtoKey);
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, RebajasRubroDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the rebajasrubro with the specified dto key in the database. 
			...
			/// <param name="rebajasrubro">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, RebajasRubroDto rebajasrubro)
			{
				try
				{
					if (dtoKey != rebajasrubro.DtoKey && dtoKey != rebajasrubro.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(rebajasrubro);
			...
			/// <summary>
			/// Deletes the rebajasrubro with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					RebajasRubroDto rebajasrubro = Read(dtoKey);
					if (rebajasrubro == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(rebajasrubro);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Returns the rebajasrubro count.
			/// </summary>
			/// <returns></returns>
			protected int Count()
			{
			...
			/// <summary>
			/// Handles adding of rebajasrubro with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="rebajasrubro">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<RebajasRubroDto> HandleCreateRebajasRubro(RebajasRubroDto rebajasrubro, UriTemplate template)
			{
				RebajasRubroDto addedRebajasRubro = this.Create(rebajasrubro);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedRebajasRubro.DtoKey, template));
				ItemInfo<RebajasRubroDto> itemInfo = new ItemInfo<RebajasRubroDto>();
				itemInfo.Item = addedRebajasRubro;
				itemInfo.EditLink = GetItemLink(addedRebajasRubro.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of rebajasrubro with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected RebajasRubroDto HandleReadRebajasRubro(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of RebajasRubros with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<RebajasRubroDto> HandleReadRebajasRubros(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.RebajasRubrosPerPageCount > maxEntities)
					{
						this.RebajasRubrosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.RebajasRubrosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.RebajasRubrosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, RebajasRubroDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.RebajasRubrosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, RebajasRubroDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<RebajasRubroDto>> itemsInfo = new List<ItemInfo<RebajasRubroDto>>();
	
					foreach (KeyValuePair<string, RebajasRubroDto> item in items)
					{
						ItemInfo<RebajasRubroDto> itemInfo = new ItemInfo<RebajasRubroDto>();
					...
					return new ItemInfoList<RebajasRubroDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of rebajasrubro with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="rebajasrubro">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected RebajasRubroDto HandleUpdateRebajasRubro(string dtoKey, RebajasRubroDto rebajasrubro)
			{
				this.Update(dtoKey, rebajasrubro);
	
				return rebajasrubro;
			}
	
			/// <summary>
			/// Handles deleting of rebajasrubro with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteRebajasRubro(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// RebajasRubrosCollectionService finalizer.
			/// </summary>
			~RebajasRubrosCollectionService()
			{
