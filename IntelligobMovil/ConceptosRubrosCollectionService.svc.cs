...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// ConceptosRubrosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class ConceptosRubrosCollectionService : IConceptosRubrosCollectionService
		{
			...
			#region IConceptosRubrosCollectionService Members
			...
			{
				return this.HandleReadConceptosRubro(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<ConceptosRubroDto> ReadConceptosRubros()
			{
				return this.HandleReadConceptosRubros(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="conceptosrubro">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<ConceptosRubroDto> CreateConceptosRubro(ConceptosRubroDto conceptosrubro)
			{
				return this.HandleCreateConceptosRubro(conceptosrubro, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="conceptosrubro">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public ConceptosRubroDto UpdateConceptosRubro(string dtoKey, ConceptosRubroDto conceptosrubro)
			{
				return this.HandleUpdateConceptosRubro(dtoKey, conceptosrubro);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteConceptosRubro(string dtoKey)
			{
				this.HandleDeleteConceptosRubro(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of ConceptosRubros returned per page.
			/// </summary>
			public int ConceptosRubrosPerPageCount
			{
				get 
				{ 
					return this.conceptosrubrosPerPageCount; 
				}
				set 
				{
					if (value != this.conceptosrubrosPerPageCount)
					{
						this.conceptosrubrosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IConceptosRubroService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new ConceptosRubroService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a conceptosrubro to the database and return the conceptosrubro with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="conceptosrubro">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected ConceptosRubroDto Create(ConceptosRubroDto conceptosrubro)
			{
				try
				{
					string dtoKey = this.Service.Add(conceptosrubro);
					this.UnitOfWork.SaveChanges();
					ConceptosRubroDto addedConceptosRubro = this.Read(dtoKey);
					if (addedConceptosRubro == null)
					{
			...
			/// <summary>
			/// Returns the ConceptosRubroDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ConceptosRubroDto Read(string dtoKey)
			{
				try
				{
					ConceptosRubroDto conceptosrubro = this.Service.GetByKey(dtoKey);
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, ConceptosRubroDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the conceptosrubro with the specified dto key in the database. 
			...
			/// <param name="conceptosrubro">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, ConceptosRubroDto conceptosrubro)
			{
				try
				{
					if (dtoKey != conceptosrubro.DtoKey && dtoKey != conceptosrubro.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(conceptosrubro);
			...
			/// <summary>
			/// Deletes the conceptosrubro with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					ConceptosRubroDto conceptosrubro = Read(dtoKey);
					if (conceptosrubro == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(conceptosrubro);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Returns the conceptosrubro count.
			/// </summary>
			/// <returns></returns>
			protected int Count()
			{
			...
			/// <summary>
			/// Handles adding of conceptosrubro with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="conceptosrubro">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<ConceptosRubroDto> HandleCreateConceptosRubro(ConceptosRubroDto conceptosrubro, UriTemplate template)
			{
				ConceptosRubroDto addedConceptosRubro = this.Create(conceptosrubro);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedConceptosRubro.DtoKey, template));
				ItemInfo<ConceptosRubroDto> itemInfo = new ItemInfo<ConceptosRubroDto>();
				itemInfo.Item = addedConceptosRubro;
				itemInfo.EditLink = GetItemLink(addedConceptosRubro.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of conceptosrubro with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ConceptosRubroDto HandleReadConceptosRubro(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of ConceptosRubros with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<ConceptosRubroDto> HandleReadConceptosRubros(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.ConceptosRubrosPerPageCount > maxEntities)
					{
						this.ConceptosRubrosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.ConceptosRubrosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.ConceptosRubrosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, ConceptosRubroDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.ConceptosRubrosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, ConceptosRubroDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<ConceptosRubroDto>> itemsInfo = new List<ItemInfo<ConceptosRubroDto>>();
	
					foreach (KeyValuePair<string, ConceptosRubroDto> item in items)
					{
						ItemInfo<ConceptosRubroDto> itemInfo = new ItemInfo<ConceptosRubroDto>();
					...
					return new ItemInfoList<ConceptosRubroDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of conceptosrubro with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="conceptosrubro">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected ConceptosRubroDto HandleUpdateConceptosRubro(string dtoKey, ConceptosRubroDto conceptosrubro)
			{
				this.Update(dtoKey, conceptosrubro);
	
				return conceptosrubro;
			}
	
			/// <summary>
			/// Handles deleting of conceptosrubro with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteConceptosRubro(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// ConceptosRubrosCollectionService finalizer.
			/// </summary>
			~ConceptosRubrosCollectionService()
			{
