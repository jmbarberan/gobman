...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// PlanillaRubrosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class PlanillaRubrosCollectionService : IPlanillaRubrosCollectionService
		{
			...
			#region IPlanillaRubrosCollectionService Members
			...
			{
				return this.HandleReadPlanillaRubro(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<PlanillaRubroDto> ReadPlanillaRubros()
			{
				return this.HandleReadPlanillaRubros(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="planillarubro">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<PlanillaRubroDto> CreatePlanillaRubro(PlanillaRubroDto planillarubro)
			{
				return this.HandleCreatePlanillaRubro(planillarubro, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="planillarubro">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public PlanillaRubroDto UpdatePlanillaRubro(string dtoKey, PlanillaRubroDto planillarubro)
			{
				return this.HandleUpdatePlanillaRubro(dtoKey, planillarubro);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeletePlanillaRubro(string dtoKey)
			{
				this.HandleDeletePlanillaRubro(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of PlanillaRubros returned per page.
			/// </summary>
			public int PlanillaRubrosPerPageCount
			{
				get 
				{ 
					return this.planillarubrosPerPageCount; 
				}
				set 
				{
					if (value != this.planillarubrosPerPageCount)
					{
						this.planillarubrosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IPlanillaRubroService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new PlanillaRubroService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a planillarubro to the database and return the planillarubro with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="planillarubro">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected PlanillaRubroDto Create(PlanillaRubroDto planillarubro)
			{
				try
				{
					string dtoKey = this.Service.Add(planillarubro);
					this.UnitOfWork.SaveChanges();
					PlanillaRubroDto addedPlanillaRubro = this.Read(dtoKey);
					if (addedPlanillaRubro == null)
					{
			...
			/// <summary>
			/// Returns the PlanillaRubroDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected PlanillaRubroDto Read(string dtoKey)
			{
				try
				{
					PlanillaRubroDto planillarubro = this.Service.GetByKey(dtoKey);
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, PlanillaRubroDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the planillarubro with the specified dto key in the database. 
			...
			/// <param name="planillarubro">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, PlanillaRubroDto planillarubro)
			{
				try
				{
					if (dtoKey != planillarubro.DtoKey && dtoKey != planillarubro.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(planillarubro);
			...
			/// <summary>
			/// Deletes the planillarubro with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					PlanillaRubroDto planillarubro = Read(dtoKey);
					if (planillarubro == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(planillarubro);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Returns the planillarubro count.
			/// </summary>
			/// <returns></returns>
			protected int Count()
			{
			...
			/// <summary>
			/// Handles adding of planillarubro with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="planillarubro">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<PlanillaRubroDto> HandleCreatePlanillaRubro(PlanillaRubroDto planillarubro, UriTemplate template)
			{
				PlanillaRubroDto addedPlanillaRubro = this.Create(planillarubro);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedPlanillaRubro.DtoKey, template));
				ItemInfo<PlanillaRubroDto> itemInfo = new ItemInfo<PlanillaRubroDto>();
				itemInfo.Item = addedPlanillaRubro;
				itemInfo.EditLink = GetItemLink(addedPlanillaRubro.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of planillarubro with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected PlanillaRubroDto HandleReadPlanillaRubro(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of PlanillaRubros with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<PlanillaRubroDto> HandleReadPlanillaRubros(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.PlanillaRubrosPerPageCount > maxEntities)
					{
						this.PlanillaRubrosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.PlanillaRubrosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.PlanillaRubrosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, PlanillaRubroDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.PlanillaRubrosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, PlanillaRubroDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<PlanillaRubroDto>> itemsInfo = new List<ItemInfo<PlanillaRubroDto>>();
	
					foreach (KeyValuePair<string, PlanillaRubroDto> item in items)
					{
						ItemInfo<PlanillaRubroDto> itemInfo = new ItemInfo<PlanillaRubroDto>();
					...
					return new ItemInfoList<PlanillaRubroDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of planillarubro with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="planillarubro">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected PlanillaRubroDto HandleUpdatePlanillaRubro(string dtoKey, PlanillaRubroDto planillarubro)
			{
				this.Update(dtoKey, planillarubro);
	
				return planillarubro;
			}
	
			/// <summary>
			/// Handles deleting of planillarubro with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeletePlanillaRubro(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// PlanillaRubrosCollectionService finalizer.
			/// </summary>
			~PlanillaRubrosCollectionService()
			{
