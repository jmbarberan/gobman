...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// ModulosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class ModulosCollectionService : IModulosCollectionService
		{
			...
			/// </summary>
			private int modulosPerPageCount = 5;
			...
			#region IModulosCollectionService Members
			...
			{
				return this.HandleReadModulo(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<ModuloDto> ReadModulos()
			{
				return this.HandleReadModulos(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="modulo">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<ModuloDto> CreateModulo(ModuloDto modulo)
			{
				return this.HandleCreateModulo(modulo, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="modulo">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public ModuloDto UpdateModulo(string dtoKey, ModuloDto modulo)
			{
				return this.HandleUpdateModulo(dtoKey, modulo);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteModulo(string dtoKey)
			{
				this.HandleDeleteModulo(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Modulos returned per page.
			/// </summary>
			public int ModulosPerPageCount
			{
				get 
				{ 
					return this.modulosPerPageCount; 
				}
				set 
				{
					if (value != this.modulosPerPageCount)
					{
						this.modulosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IModuloService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new ModuloService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a modulo to the database and return the modulo with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="modulo">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected ModuloDto Create(ModuloDto modulo)
			{
				try
				{
					string dtoKey = this.Service.Add(modulo);
					this.UnitOfWork.SaveChanges();
					ModuloDto addedModulo = this.Read(dtoKey);
					if (addedModulo == null)
					{
			...
			/// <summary>
			/// Returns the ModuloDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ModuloDto Read(string dtoKey)
			{
				try
				{
					ModuloDto modulo = this.Service.GetByKey(dtoKey);
					if (modulo == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, ModuloDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the modulo with the specified dto key in the database. 
			...
			/// <param name="modulo">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, ModuloDto modulo)
			{
				try
				{
					if (dtoKey != modulo.DtoKey && dtoKey != modulo.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
			...
			/// <summary>
			/// Deletes the modulo with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			/// <summary>
			/// Handles adding of modulo with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="modulo">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<ModuloDto> HandleCreateModulo(ModuloDto modulo, UriTemplate template)
			{
				ModuloDto addedModulo = this.Create(modulo);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedModulo.DtoKey, template));
				ItemInfo<ModuloDto> itemInfo = new ItemInfo<ModuloDto>();
				itemInfo.Item = addedModulo;
				itemInfo.EditLink = GetItemLink(addedModulo.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of modulo with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ModuloDto HandleReadModulo(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Modulos with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<ModuloDto> HandleReadModulos(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.ModulosPerPageCount > maxEntities)
					{
						this.ModulosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.ModulosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.ModulosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, ModuloDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.ModulosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, ModuloDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<ModuloDto>> itemsInfo = new List<ItemInfo<ModuloDto>>();
	
					foreach (KeyValuePair<string, ModuloDto> item in items)
					{
						ItemInfo<ModuloDto> itemInfo = new ItemInfo<ModuloDto>();
					...
					return new ItemInfoList<ModuloDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of modulo with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="modulo">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected ModuloDto HandleUpdateModulo(string dtoKey, ModuloDto modulo)
			{
				this.Update(dtoKey, modulo);
	
				return modulo;
			}
	
			/// <summary>
			/// Handles deleting of modulo with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteModulo(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// ModulosCollectionService finalizer.
			/// </summary>
			~ModulosCollectionService()
			{
