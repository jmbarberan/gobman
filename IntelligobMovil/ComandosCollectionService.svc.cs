...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// ComandosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class ComandosCollectionService : IComandosCollectionService
		{
			...
			#region IComandosCollectionService Members
			...
			{
				return this.HandleReadComando(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<ComandoDto> ReadComandos()
			{
				return this.HandleReadComandos(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="comando">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<ComandoDto> CreateComando(ComandoDto comando)
			{
				return this.HandleCreateComando(comando, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="comando">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public ComandoDto UpdateComando(string dtoKey, ComandoDto comando)
			{
				return this.HandleUpdateComando(dtoKey, comando);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteComando(string dtoKey)
			{
				this.HandleDeleteComando(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Comandos returned per page.
			/// </summary>
			public int ComandosPerPageCount
			{
				get 
				{ 
					return this.comandosPerPageCount; 
				}
				set 
				{
					if (value != this.comandosPerPageCount)
					{
						this.comandosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IComandoService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new ComandoService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a comando to the database and return the comando with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="comando">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected ComandoDto Create(ComandoDto comando)
			{
				try
				{
					string dtoKey = this.Service.Add(comando);
					this.UnitOfWork.SaveChanges();
					ComandoDto addedComando = this.Read(dtoKey);
					if (addedComando == null)
					{
			...
			/// <summary>
			/// Returns the ComandoDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ComandoDto Read(string dtoKey)
			{
				try
				{
					ComandoDto comando = this.Service.GetByKey(dtoKey);
					if (comando == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, ComandoDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the comando with the specified dto key in the database. 
			...
			/// <param name="comando">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, ComandoDto comando)
			{
				try
				{
					if (dtoKey != comando.DtoKey && dtoKey != comando.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
			...
			/// <summary>
			/// Deletes the comando with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					ComandoDto comando = Read(dtoKey);
					if (comando == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
			...
			/// <summary>
			/// Handles adding of comando with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="comando">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<ComandoDto> HandleCreateComando(ComandoDto comando, UriTemplate template)
			{
				ComandoDto addedComando = this.Create(comando);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedComando.DtoKey, template));
				ItemInfo<ComandoDto> itemInfo = new ItemInfo<ComandoDto>();
				itemInfo.Item = addedComando;
				itemInfo.EditLink = GetItemLink(addedComando.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of comando with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ComandoDto HandleReadComando(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Comandos with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<ComandoDto> HandleReadComandos(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.ComandosPerPageCount > maxEntities)
					{
						this.ComandosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.ComandosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.ComandosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, ComandoDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.ComandosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, ComandoDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<ComandoDto>> itemsInfo = new List<ItemInfo<ComandoDto>>();
	
					foreach (KeyValuePair<string, ComandoDto> item in items)
					{
						ItemInfo<ComandoDto> itemInfo = new ItemInfo<ComandoDto>();
					...
					return new ItemInfoList<ComandoDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of comando with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="comando">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected ComandoDto HandleUpdateComando(string dtoKey, ComandoDto comando)
			{
				this.Update(dtoKey, comando);
	
				return comando;
			}
	
			/// <summary>
			/// Handles deleting of comando with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteComando(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// ComandosCollectionService finalizer.
			/// </summary>
			~ComandosCollectionService()
			{
