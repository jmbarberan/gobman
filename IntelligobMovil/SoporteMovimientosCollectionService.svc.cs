...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// SoporteMovimientosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class SoporteMovimientosCollectionService : ISoporteMovimientosCollectionService
		{
			...
			#region ISoporteMovimientosCollectionService Members
			...
			{
				return this.HandleReadSoporteMovimiento(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<SoporteMovimientoDto> ReadSoporteMovimientos()
			{
				return this.HandleReadSoporteMovimientos(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="soportemovimiento">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<SoporteMovimientoDto> CreateSoporteMovimiento(SoporteMovimientoDto soportemovimiento)
			{
				return this.HandleCreateSoporteMovimiento(soportemovimiento, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="soportemovimiento">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public SoporteMovimientoDto UpdateSoporteMovimiento(string dtoKey, SoporteMovimientoDto soportemovimiento)
			{
				return this.HandleUpdateSoporteMovimiento(dtoKey, soportemovimiento);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteSoporteMovimiento(string dtoKey)
			{
				this.HandleDeleteSoporteMovimiento(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of SoporteMovimientos returned per page.
			/// </summary>
			public int SoporteMovimientosPerPageCount
			{
				get 
				{ 
					return this.soportemovimientosPerPageCount; 
				}
				set 
				{
					if (value != this.soportemovimientosPerPageCount)
					{
						this.soportemovimientosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual ISoporteMovimientoService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new SoporteMovimientoService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a soportemovimiento to the database and return the soportemovimiento with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="soportemovimiento">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected SoporteMovimientoDto Create(SoporteMovimientoDto soportemovimiento)
			{
				try
				{
					string dtoKey = this.Service.Add(soportemovimiento);
					this.UnitOfWork.SaveChanges();
					SoporteMovimientoDto addedSoporteMovimiento = this.Read(dtoKey);
					if (addedSoporteMovimiento == null)
					{
			...
			/// <summary>
			/// Returns the SoporteMovimientoDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected SoporteMovimientoDto Read(string dtoKey)
			{
				try
				{
					SoporteMovimientoDto soportemovimiento = this.Service.GetByKey(dtoKey);
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, SoporteMovimientoDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the soportemovimiento with the specified dto key in the database. 
			...
			/// <param name="soportemovimiento">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, SoporteMovimientoDto soportemovimiento)
			{
				try
				{
					if (dtoKey != soportemovimiento.DtoKey && dtoKey != soportemovimiento.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(soportemovimiento);
			...
			/// <summary>
			/// Deletes the soportemovimiento with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					SoporteMovimientoDto soportemovimiento = Read(dtoKey);
					if (soportemovimiento == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(soportemovimiento);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Returns the soportemovimiento count.
			/// </summary>
			/// <returns></returns>
			protected int Count()
			{
			...
			/// <summary>
			/// Handles adding of soportemovimiento with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="soportemovimiento">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<SoporteMovimientoDto> HandleCreateSoporteMovimiento(SoporteMovimientoDto soportemovimiento, UriTemplate template)
			{
				SoporteMovimientoDto addedSoporteMovimiento = this.Create(soportemovimiento);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedSoporteMovimiento.DtoKey, template));
				ItemInfo<SoporteMovimientoDto> itemInfo = new ItemInfo<SoporteMovimientoDto>();
				itemInfo.Item = addedSoporteMovimiento;
				itemInfo.EditLink = GetItemLink(addedSoporteMovimiento.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of soportemovimiento with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected SoporteMovimientoDto HandleReadSoporteMovimiento(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of SoporteMovimientos with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<SoporteMovimientoDto> HandleReadSoporteMovimientos(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.SoporteMovimientosPerPageCount > maxEntities)
					{
						this.SoporteMovimientosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.SoporteMovimientosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.SoporteMovimientosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, SoporteMovimientoDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.SoporteMovimientosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, SoporteMovimientoDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<SoporteMovimientoDto>> itemsInfo = new List<ItemInfo<SoporteMovimientoDto>>();
	
					foreach (KeyValuePair<string, SoporteMovimientoDto> item in items)
					{
						ItemInfo<SoporteMovimientoDto> itemInfo = new ItemInfo<SoporteMovimientoDto>();
					...
					return new ItemInfoList<SoporteMovimientoDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of soportemovimiento with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="soportemovimiento">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected SoporteMovimientoDto HandleUpdateSoporteMovimiento(string dtoKey, SoporteMovimientoDto soportemovimiento)
			{
				this.Update(dtoKey, soportemovimiento);
	
				return soportemovimiento;
			}
	
			/// <summary>
			/// Handles deleting of soportemovimiento with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteSoporteMovimiento(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// SoporteMovimientosCollectionService finalizer.
			/// </summary>
			~SoporteMovimientosCollectionService()
			{
