...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// PredioFotosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class PredioFotosCollectionService : IPredioFotosCollectionService
		{
			...
			#region IPredioFotosCollectionService Members
			...
			{
				return this.HandleReadPredioFoto(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<PredioFotoDto> ReadPredioFotos()
			{
				return this.HandleReadPredioFotos(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="prediofoto">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<PredioFotoDto> CreatePredioFoto(PredioFotoDto prediofoto)
			{
				return this.HandleCreatePredioFoto(prediofoto, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="prediofoto">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public PredioFotoDto UpdatePredioFoto(string dtoKey, PredioFotoDto prediofoto)
			{
				return this.HandleUpdatePredioFoto(dtoKey, prediofoto);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeletePredioFoto(string dtoKey)
			{
				this.HandleDeletePredioFoto(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of PredioFotos returned per page.
			/// </summary>
			public int PredioFotosPerPageCount
			{
				get 
				{ 
					return this.prediofotosPerPageCount; 
				}
				set 
				{
					if (value != this.prediofotosPerPageCount)
					{
						this.prediofotosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IPredioFotoService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new PredioFotoService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a prediofoto to the database and return the prediofoto with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="prediofoto">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected PredioFotoDto Create(PredioFotoDto prediofoto)
			{
				try
				{
					string dtoKey = this.Service.Add(prediofoto);
					this.UnitOfWork.SaveChanges();
					PredioFotoDto addedPredioFoto = this.Read(dtoKey);
					if (addedPredioFoto == null)
					{
			...
			/// <summary>
			/// Returns the PredioFotoDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected PredioFotoDto Read(string dtoKey)
			{
				try
				{
					PredioFotoDto prediofoto = this.Service.GetByKey(dtoKey);
					if (prediofoto == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, PredioFotoDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the prediofoto with the specified dto key in the database. 
			...
			/// <param name="prediofoto">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, PredioFotoDto prediofoto)
			{
				try
				{
					if (dtoKey != prediofoto.DtoKey && dtoKey != prediofoto.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(prediofoto);
			...
			/// <summary>
			/// Deletes the prediofoto with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					PredioFotoDto prediofoto = Read(dtoKey);
					if (prediofoto == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(prediofoto);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Handles adding of prediofoto with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="prediofoto">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<PredioFotoDto> HandleCreatePredioFoto(PredioFotoDto prediofoto, UriTemplate template)
			{
				PredioFotoDto addedPredioFoto = this.Create(prediofoto);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedPredioFoto.DtoKey, template));
				ItemInfo<PredioFotoDto> itemInfo = new ItemInfo<PredioFotoDto>();
				itemInfo.Item = addedPredioFoto;
				itemInfo.EditLink = GetItemLink(addedPredioFoto.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of prediofoto with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected PredioFotoDto HandleReadPredioFoto(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of PredioFotos with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<PredioFotoDto> HandleReadPredioFotos(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.PredioFotosPerPageCount > maxEntities)
					{
						this.PredioFotosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.PredioFotosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.PredioFotosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, PredioFotoDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.PredioFotosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, PredioFotoDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<PredioFotoDto>> itemsInfo = new List<ItemInfo<PredioFotoDto>>();
	
					foreach (KeyValuePair<string, PredioFotoDto> item in items)
					{
						ItemInfo<PredioFotoDto> itemInfo = new ItemInfo<PredioFotoDto>();
					...
					return new ItemInfoList<PredioFotoDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of prediofoto with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="prediofoto">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected PredioFotoDto HandleUpdatePredioFoto(string dtoKey, PredioFotoDto prediofoto)
			{
				this.Update(dtoKey, prediofoto);
	
				return prediofoto;
			}
	
			/// <summary>
			/// Handles deleting of prediofoto with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeletePredioFoto(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// PredioFotosCollectionService finalizer.
			/// </summary>
			~PredioFotosCollectionService()
			{
