...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// PredioBloquesCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class PredioBloquesCollectionService : IPredioBloquesCollectionService
		{
			...
			#region IPredioBloquesCollectionService Members
			...
			{
				return this.HandleReadPredioBloque(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<PredioBloqueDto> ReadPredioBloques()
			{
				return this.HandleReadPredioBloques(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="prediobloque">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<PredioBloqueDto> CreatePredioBloque(PredioBloqueDto prediobloque)
			{
				return this.HandleCreatePredioBloque(prediobloque, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="prediobloque">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public PredioBloqueDto UpdatePredioBloque(string dtoKey, PredioBloqueDto prediobloque)
			{
				return this.HandleUpdatePredioBloque(dtoKey, prediobloque);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeletePredioBloque(string dtoKey)
			{
				this.HandleDeletePredioBloque(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of PredioBloques returned per page.
			/// </summary>
			public int PredioBloquesPerPageCount
			{
				get 
				{ 
					return this.prediobloquesPerPageCount; 
				}
				set 
				{
					if (value != this.prediobloquesPerPageCount)
					{
						this.prediobloquesPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IPredioBloqueService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new PredioBloqueService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a prediobloque to the database and return the prediobloque with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="prediobloque">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected PredioBloqueDto Create(PredioBloqueDto prediobloque)
			{
				try
				{
					string dtoKey = this.Service.Add(prediobloque);
					this.UnitOfWork.SaveChanges();
					PredioBloqueDto addedPredioBloque = this.Read(dtoKey);
					if (addedPredioBloque == null)
					{
			...
			/// <summary>
			/// Returns the PredioBloqueDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected PredioBloqueDto Read(string dtoKey)
			{
				try
				{
					PredioBloqueDto prediobloque = this.Service.GetByKey(dtoKey);
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, PredioBloqueDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the prediobloque with the specified dto key in the database. 
			...
			/// <param name="prediobloque">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, PredioBloqueDto prediobloque)
			{
				try
				{
					if (dtoKey != prediobloque.DtoKey && dtoKey != prediobloque.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(prediobloque);
			...
			/// <summary>
			/// Deletes the prediobloque with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					PredioBloqueDto prediobloque = Read(dtoKey);
					if (prediobloque == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(prediobloque);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Returns the prediobloque count.
			/// </summary>
			/// <returns></returns>
			protected int Count()
			{
			...
			/// <summary>
			/// Handles adding of prediobloque with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="prediobloque">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<PredioBloqueDto> HandleCreatePredioBloque(PredioBloqueDto prediobloque, UriTemplate template)
			{
				PredioBloqueDto addedPredioBloque = this.Create(prediobloque);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedPredioBloque.DtoKey, template));
				ItemInfo<PredioBloqueDto> itemInfo = new ItemInfo<PredioBloqueDto>();
				itemInfo.Item = addedPredioBloque;
				itemInfo.EditLink = GetItemLink(addedPredioBloque.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of prediobloque with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected PredioBloqueDto HandleReadPredioBloque(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of PredioBloques with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<PredioBloqueDto> HandleReadPredioBloques(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.PredioBloquesPerPageCount > maxEntities)
					{
						this.PredioBloquesPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.PredioBloquesPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.PredioBloquesPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, PredioBloqueDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.PredioBloquesPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, PredioBloqueDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<PredioBloqueDto>> itemsInfo = new List<ItemInfo<PredioBloqueDto>>();
	
					foreach (KeyValuePair<string, PredioBloqueDto> item in items)
					{
						ItemInfo<PredioBloqueDto> itemInfo = new ItemInfo<PredioBloqueDto>();
					...
					return new ItemInfoList<PredioBloqueDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of prediobloque with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="prediobloque">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected PredioBloqueDto HandleUpdatePredioBloque(string dtoKey, PredioBloqueDto prediobloque)
			{
				this.Update(dtoKey, prediobloque);
	
				return prediobloque;
			}
	
			/// <summary>
			/// Handles deleting of prediobloque with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeletePredioBloque(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// PredioBloquesCollectionService finalizer.
			/// </summary>
			~PredioBloquesCollectionService()
			{
