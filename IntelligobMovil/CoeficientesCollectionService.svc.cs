...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// CoeficientesCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class CoeficientesCollectionService : ICoeficientesCollectionService
		{
			...
			#region ICoeficientesCollectionService Members
			...
			{
				return this.HandleReadCoeficiente(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<CoeficienteDto> ReadCoeficientes()
			{
				return this.HandleReadCoeficientes(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="coeficiente">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<CoeficienteDto> CreateCoeficiente(CoeficienteDto coeficiente)
			{
				return this.HandleCreateCoeficiente(coeficiente, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="coeficiente">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public CoeficienteDto UpdateCoeficiente(string dtoKey, CoeficienteDto coeficiente)
			{
				return this.HandleUpdateCoeficiente(dtoKey, coeficiente);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteCoeficiente(string dtoKey)
			{
				this.HandleDeleteCoeficiente(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Coeficientes returned per page.
			/// </summary>
			public int CoeficientesPerPageCount
			{
				get 
				{ 
					return this.coeficientesPerPageCount; 
				}
				set 
				{
					if (value != this.coeficientesPerPageCount)
					{
						this.coeficientesPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual ICoeficienteService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new CoeficienteService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a coeficiente to the database and return the coeficiente with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="coeficiente">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected CoeficienteDto Create(CoeficienteDto coeficiente)
			{
				try
				{
					string dtoKey = this.Service.Add(coeficiente);
					this.UnitOfWork.SaveChanges();
					CoeficienteDto addedCoeficiente = this.Read(dtoKey);
					if (addedCoeficiente == null)
					{
			...
			/// <summary>
			/// Returns the CoeficienteDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected CoeficienteDto Read(string dtoKey)
			{
				try
				{
					CoeficienteDto coeficiente = this.Service.GetByKey(dtoKey);
					if (coeficiente == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, CoeficienteDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the coeficiente with the specified dto key in the database. 
			...
			/// <param name="coeficiente">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, CoeficienteDto coeficiente)
			{
				try
				{
					if (dtoKey != coeficiente.DtoKey && dtoKey != coeficiente.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(coeficiente);
			...
			/// <summary>
			/// Deletes the coeficiente with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					CoeficienteDto coeficiente = Read(dtoKey);
					if (coeficiente == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(coeficiente);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Handles adding of coeficiente with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="coeficiente">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<CoeficienteDto> HandleCreateCoeficiente(CoeficienteDto coeficiente, UriTemplate template)
			{
				CoeficienteDto addedCoeficiente = this.Create(coeficiente);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedCoeficiente.DtoKey, template));
				ItemInfo<CoeficienteDto> itemInfo = new ItemInfo<CoeficienteDto>();
				itemInfo.Item = addedCoeficiente;
				itemInfo.EditLink = GetItemLink(addedCoeficiente.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of coeficiente with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected CoeficienteDto HandleReadCoeficiente(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Coeficientes with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<CoeficienteDto> HandleReadCoeficientes(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.CoeficientesPerPageCount > maxEntities)
					{
						this.CoeficientesPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.CoeficientesPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.CoeficientesPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, CoeficienteDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.CoeficientesPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, CoeficienteDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<CoeficienteDto>> itemsInfo = new List<ItemInfo<CoeficienteDto>>();
	
					foreach (KeyValuePair<string, CoeficienteDto> item in items)
					{
						ItemInfo<CoeficienteDto> itemInfo = new ItemInfo<CoeficienteDto>();
					...
					return new ItemInfoList<CoeficienteDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of coeficiente with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="coeficiente">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected CoeficienteDto HandleUpdateCoeficiente(string dtoKey, CoeficienteDto coeficiente)
			{
				this.Update(dtoKey, coeficiente);
	
				return coeficiente;
			}
	
			/// <summary>
			/// Handles deleting of coeficiente with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteCoeficiente(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// CoeficientesCollectionService finalizer.
			/// </summary>
			~CoeficientesCollectionService()
			{
