...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// ContribuyentesCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class ContribuyentesCollectionService : IContribuyentesCollectionService
		{
			...
			/// </summary>
			private int contribuyentesPerPageCount = 5;
			...
			#region IContribuyentesCollectionService Members
			...
			{
				return this.HandleReadContribuyente(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<ContribuyenteDto> ReadContribuyentes()
			{
				return this.HandleReadContribuyentes(itemUriTemplate);
			}
			...
			/// <returns></returns>
			public ItemInfo<ContribuyenteDto> CreateContribuyente(ContribuyenteDto contribuyente)
			{
				return this.HandleCreateContribuyente(contribuyente, itemUriTemplate);
			}
			...
			/// <returns></returns>
			public ContribuyenteDto UpdateContribuyente(string dtoKey, ContribuyenteDto contribuyente)
			{
				return this.HandleUpdateContribuyente(dtoKey, contribuyente);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteContribuyente(string dtoKey)
			{
				this.HandleDeleteContribuyente(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Contribuyentes returned per page.
			/// </summary>
			public int ContribuyentesPerPageCount
			{
				get 
				{ 
					return this.contribuyentesPerPageCount; 
				}
				set 
				{
					if (value != this.contribuyentesPerPageCount)
					{
						this.contribuyentesPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IContribuyenteService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new ContribuyenteService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a contribuyente to the database and return the contribuyente with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="contribuyente">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected ContribuyenteDto Create(ContribuyenteDto contribuyente)
			{
				try
				{
					string dtoKey = this.Service.Add(contribuyente);
					this.UnitOfWork.SaveChanges();
					ContribuyenteDto addedContribuyente = this.Read(dtoKey);
					if (addedContribuyente == null)
					{
			...
			/// <summary>
			/// Returns the ContribuyenteDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ContribuyenteDto Read(string dtoKey)
			{
				try
				{
					ContribuyenteDto contribuyente = this.Service.GetByKey(dtoKey);
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, ContribuyenteDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the contribuyente with the specified dto key in the database. 
			...
			/// <param name="contribuyente">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, ContribuyenteDto contribuyente)
			{
				try
				{
					if (dtoKey != contribuyente.DtoKey && dtoKey != contribuyente.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(contribuyente);
			...
			/// <summary>
			/// Deletes the contribuyente with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					ContribuyenteDto contribuyente = Read(dtoKey);
					if (contribuyente == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(contribuyente);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Returns the contribuyente count.
			/// </summary>
			/// <returns></returns>
			protected int Count()
			{
			...
			/// <summary>
			/// Handles adding of contribuyente with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="contribuyente">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<ContribuyenteDto> HandleCreateContribuyente(ContribuyenteDto contribuyente, UriTemplate template)
			{
				ContribuyenteDto addedContribuyente = this.Create(contribuyente);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedContribuyente.DtoKey, template));
				ItemInfo<ContribuyenteDto> itemInfo = new ItemInfo<ContribuyenteDto>();
				itemInfo.Item = addedContribuyente;
				itemInfo.EditLink = GetItemLink(addedContribuyente.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of contribuyente with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ContribuyenteDto HandleReadContribuyente(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Contribuyentes with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<ContribuyenteDto> HandleReadContribuyentes(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.ContribuyentesPerPageCount > maxEntities)
					{
						this.ContribuyentesPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.ContribuyentesPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.ContribuyentesPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, ContribuyenteDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.ContribuyentesPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, ContribuyenteDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<ContribuyenteDto>> itemsInfo = new List<ItemInfo<ContribuyenteDto>>();
	
					foreach (KeyValuePair<string, ContribuyenteDto> item in items)
					{
						ItemInfo<ContribuyenteDto> itemInfo = new ItemInfo<ContribuyenteDto>();
					...
					return new ItemInfoList<ContribuyenteDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of contribuyente with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="contribuyente">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected ContribuyenteDto HandleUpdateContribuyente(string dtoKey, ContribuyenteDto contribuyente)
			{
				this.Update(dtoKey, contribuyente);
	
				return contribuyente;
			}
	
			/// <summary>
			/// Handles deleting of contribuyente with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteContribuyente(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// ContribuyentesCollectionService finalizer.
			/// </summary>
			~ContribuyentesCollectionService()
			{
