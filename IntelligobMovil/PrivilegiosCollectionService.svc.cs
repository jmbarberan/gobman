...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// PrivilegiosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class PrivilegiosCollectionService : IPrivilegiosCollectionService
		{
			...
			#region IPrivilegiosCollectionService Members
			...
			{
				return this.HandleReadPrivilegio(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<PrivilegioDto> ReadPrivilegios()
			{
				return this.HandleReadPrivilegios(itemUriTemplate);
			}
			...
			/// <returns></returns>
			public ItemInfo<PrivilegioDto> CreatePrivilegio(PrivilegioDto privilegio)
			{
				return this.HandleCreatePrivilegio(privilegio, itemUriTemplate);
			}
			...
			/// <returns></returns>
			public PrivilegioDto UpdatePrivilegio(string dtoKey, PrivilegioDto privilegio)
			{
				return this.HandleUpdatePrivilegio(dtoKey, privilegio);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeletePrivilegio(string dtoKey)
			{
				this.HandleDeletePrivilegio(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Privilegios returned per page.
			/// </summary>
			public int PrivilegiosPerPageCount
			{
				get 
				{ 
					return this.privilegiosPerPageCount; 
				}
				set 
				{
					if (value != this.privilegiosPerPageCount)
					{
						this.privilegiosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IPrivilegioService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new PrivilegioService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a privilegio to the database and return the privilegio with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="privilegio">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected PrivilegioDto Create(PrivilegioDto privilegio)
			{
				try
				{
					string dtoKey = this.Service.Add(privilegio);
					this.UnitOfWork.SaveChanges();
					PrivilegioDto addedPrivilegio = this.Read(dtoKey);
					if (addedPrivilegio == null)
					{
			...
			/// <summary>
			/// Returns the PrivilegioDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected PrivilegioDto Read(string dtoKey)
			{
				try
				{
					PrivilegioDto privilegio = this.Service.GetByKey(dtoKey);
					if (privilegio == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, PrivilegioDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the privilegio with the specified dto key in the database. 
			...
			/// <param name="privilegio">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, PrivilegioDto privilegio)
			{
				try
				{
					if (dtoKey != privilegio.DtoKey && dtoKey != privilegio.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(privilegio);
			...
			/// <summary>
			/// Deletes the privilegio with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					PrivilegioDto privilegio = Read(dtoKey);
					if (privilegio == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(privilegio);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Handles adding of privilegio with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="privilegio">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<PrivilegioDto> HandleCreatePrivilegio(PrivilegioDto privilegio, UriTemplate template)
			{
				PrivilegioDto addedPrivilegio = this.Create(privilegio);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedPrivilegio.DtoKey, template));
				ItemInfo<PrivilegioDto> itemInfo = new ItemInfo<PrivilegioDto>();
				itemInfo.Item = addedPrivilegio;
				itemInfo.EditLink = GetItemLink(addedPrivilegio.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of privilegio with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected PrivilegioDto HandleReadPrivilegio(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Privilegios with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<PrivilegioDto> HandleReadPrivilegios(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.PrivilegiosPerPageCount > maxEntities)
					{
						this.PrivilegiosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.PrivilegiosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.PrivilegiosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, PrivilegioDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.PrivilegiosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, PrivilegioDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<PrivilegioDto>> itemsInfo = new List<ItemInfo<PrivilegioDto>>();
	
					foreach (KeyValuePair<string, PrivilegioDto> item in items)
					{
						ItemInfo<PrivilegioDto> itemInfo = new ItemInfo<PrivilegioDto>();
					...
					return new ItemInfoList<PrivilegioDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of privilegio with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="privilegio">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected PrivilegioDto HandleUpdatePrivilegio(string dtoKey, PrivilegioDto privilegio)
			{
				this.Update(dtoKey, privilegio);
	
				return privilegio;
			}
	
			/// <summary>
			/// Handles deleting of privilegio with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeletePrivilegio(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// PrivilegiosCollectionService finalizer.
			/// </summary>
			~PrivilegiosCollectionService()
			{
