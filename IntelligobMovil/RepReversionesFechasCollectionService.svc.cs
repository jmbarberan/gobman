...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// RepReversionesFechasCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class RepReversionesFechasCollectionService : IRepReversionesFechasCollectionService
		{
			...
			#region IRepReversionesFechasCollectionService Members
			...
			{
				return this.HandleReadRepReversionesFecha(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<RepReversionesFechaDto> ReadRepReversionesFechas()
			{
				return this.HandleReadRepReversionesFechas(itemUriTemplate);
			}
			...
			/// <returns></returns>
			public ItemInfo<RepReversionesFechaDto> CreateRepReversionesFecha(RepReversionesFechaDto repreversionesfecha)
			{
				return this.HandleCreateRepReversionesFecha(repreversionesfecha, itemUriTemplate);
			}
			...
			/// <returns></returns>
			public RepReversionesFechaDto UpdateRepReversionesFecha(string dtoKey, RepReversionesFechaDto repreversionesfecha)
			{
				return this.HandleUpdateRepReversionesFecha(dtoKey, repreversionesfecha);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteRepReversionesFecha(string dtoKey)
			{
				this.HandleDeleteRepReversionesFecha(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of RepReversionesFechas returned per page.
			/// </summary>
			public int RepReversionesFechasPerPageCount
			{
				get 
				{ 
					return this.repreversionesfechasPerPageCount; 
				}
				set 
				{
					if (value != this.repreversionesfechasPerPageCount)
					{
						this.repreversionesfechasPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IRepReversionesFechaService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new RepReversionesFechaService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a repreversionesfecha to the database and return the repreversionesfecha with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="repreversionesfecha">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected RepReversionesFechaDto Create(RepReversionesFechaDto repreversionesfecha)
			{
				try
				{
					string dtoKey = this.Service.Add(repreversionesfecha);
					this.UnitOfWork.SaveChanges();
					RepReversionesFechaDto addedRepReversionesFecha = this.Read(dtoKey);
					if (addedRepReversionesFecha == null)
					{
					...
					return addedRepReversionesFecha;
				}
			...
			/// <summary>
			/// Returns the RepReversionesFechaDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected RepReversionesFechaDto Read(string dtoKey)
			{
				try
				{
					RepReversionesFechaDto repreversionesfecha = this.Service.GetByKey(dtoKey);
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, RepReversionesFechaDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the repreversionesfecha with the specified dto key in the database. 
			...
			/// <param name="repreversionesfecha">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, RepReversionesFechaDto repreversionesfecha)
			{
				try
				{
					if (dtoKey != repreversionesfecha.DtoKey && dtoKey != repreversionesfecha.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(repreversionesfecha);
			...
			/// <summary>
			/// Deletes the repreversionesfecha with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					RepReversionesFechaDto repreversionesfecha = Read(dtoKey);
					if (repreversionesfecha == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(repreversionesfecha);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Returns the repreversionesfecha count.
			/// </summary>
			/// <returns></returns>
			protected int Count()
			{
			...
			/// <summary>
			/// Handles adding of repreversionesfecha with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="repreversionesfecha">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<RepReversionesFechaDto> HandleCreateRepReversionesFecha(RepReversionesFechaDto repreversionesfecha, UriTemplate template)
			{
				RepReversionesFechaDto addedRepReversionesFecha = this.Create(repreversionesfecha);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedRepReversionesFecha.DtoKey, template));
				ItemInfo<RepReversionesFechaDto> itemInfo = new ItemInfo<RepReversionesFechaDto>();
				itemInfo.Item = addedRepReversionesFecha;
				itemInfo.EditLink = GetItemLink(addedRepReversionesFecha.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of repreversionesfecha with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected RepReversionesFechaDto HandleReadRepReversionesFecha(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of RepReversionesFechas with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<RepReversionesFechaDto> HandleReadRepReversionesFechas(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.RepReversionesFechasPerPageCount > maxEntities)
					{
						this.RepReversionesFechasPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.RepReversionesFechasPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.RepReversionesFechasPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, RepReversionesFechaDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.RepReversionesFechasPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, RepReversionesFechaDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<RepReversionesFechaDto>> itemsInfo = new List<ItemInfo<RepReversionesFechaDto>>();
	
					foreach (KeyValuePair<string, RepReversionesFechaDto> item in items)
					{
						ItemInfo<RepReversionesFechaDto> itemInfo = new ItemInfo<RepReversionesFechaDto>();
					...
					return new ItemInfoList<RepReversionesFechaDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of repreversionesfecha with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="repreversionesfecha">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected RepReversionesFechaDto HandleUpdateRepReversionesFecha(string dtoKey, RepReversionesFechaDto repreversionesfecha)
			{
				this.Update(dtoKey, repreversionesfecha);
	
				return repreversionesfecha;
			}
	
			/// <summary>
			/// Handles deleting of repreversionesfecha with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteRepReversionesFecha(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// RepReversionesFechasCollectionService finalizer.
			/// </summary>
			~RepReversionesFechasCollectionService()
			{
