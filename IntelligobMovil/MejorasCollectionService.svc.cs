...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// MejorasCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class MejorasCollectionService : IMejorasCollectionService
		{
			...
			#region IMejorasCollectionService Members
			...
			{
				return this.HandleReadMejora(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<MejoraDto> ReadMejoras()
			{
				return this.HandleReadMejoras(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="mejora">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<MejoraDto> CreateMejora(MejoraDto mejora)
			{
				return this.HandleCreateMejora(mejora, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="mejora">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public MejoraDto UpdateMejora(string dtoKey, MejoraDto mejora)
			{
				return this.HandleUpdateMejora(dtoKey, mejora);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteMejora(string dtoKey)
			{
				this.HandleDeleteMejora(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Mejoras returned per page.
			/// </summary>
			public int MejorasPerPageCount
			{
				get 
				{ 
					return this.mejorasPerPageCount; 
				}
				set 
				{
					if (value != this.mejorasPerPageCount)
					{
						this.mejorasPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IMejoraService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new MejoraService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a mejora to the database and return the mejora with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="mejora">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected MejoraDto Create(MejoraDto mejora)
			{
				try
				{
					string dtoKey = this.Service.Add(mejora);
					this.UnitOfWork.SaveChanges();
					MejoraDto addedMejora = this.Read(dtoKey);
					if (addedMejora == null)
					{
			...
			/// <summary>
			/// Returns the MejoraDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected MejoraDto Read(string dtoKey)
			{
				try
				{
					MejoraDto mejora = this.Service.GetByKey(dtoKey);
					if (mejora == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, MejoraDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the mejora with the specified dto key in the database. 
			...
			/// <param name="mejora">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, MejoraDto mejora)
			{
				try
				{
					if (dtoKey != mejora.DtoKey && dtoKey != mejora.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
			...
			/// <summary>
			/// Deletes the mejora with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			/// <summary>
			/// Handles adding of mejora with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="mejora">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<MejoraDto> HandleCreateMejora(MejoraDto mejora, UriTemplate template)
			{
				MejoraDto addedMejora = this.Create(mejora);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedMejora.DtoKey, template));
				ItemInfo<MejoraDto> itemInfo = new ItemInfo<MejoraDto>();
				itemInfo.Item = addedMejora;
				itemInfo.EditLink = GetItemLink(addedMejora.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of mejora with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected MejoraDto HandleReadMejora(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Mejoras with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<MejoraDto> HandleReadMejoras(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.MejorasPerPageCount > maxEntities)
					{
						this.MejorasPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.MejorasPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.MejorasPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, MejoraDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.MejorasPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, MejoraDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<MejoraDto>> itemsInfo = new List<ItemInfo<MejoraDto>>();
	
					foreach (KeyValuePair<string, MejoraDto> item in items)
					{
						ItemInfo<MejoraDto> itemInfo = new ItemInfo<MejoraDto>();
					...
					return new ItemInfoList<MejoraDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of mejora with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="mejora">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected MejoraDto HandleUpdateMejora(string dtoKey, MejoraDto mejora)
			{
				this.Update(dtoKey, mejora);
	
				return mejora;
			}
	
			/// <summary>
			/// Handles deleting of mejora with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteMejora(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// MejorasCollectionService finalizer.
			/// </summary>
			~MejorasCollectionService()
			{
