...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// RubrosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class RubrosCollectionService : IRubrosCollectionService
		{
			...
			/// </summary>
			private int rubrosPerPageCount = 5;
			...
			#region IRubrosCollectionService Members
			...
			{
				return this.HandleReadRubro(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<RubroDto> ReadRubros()
			{
				return this.HandleReadRubros(itemUriTemplate);
			}
			...
			/// <returns></returns>
			public ItemInfo<RubroDto> CreateRubro(RubroDto rubro)
			{
				return this.HandleCreateRubro(rubro, itemUriTemplate);
			}
			...
			/// <returns></returns>
			public RubroDto UpdateRubro(string dtoKey, RubroDto rubro)
			{
				return this.HandleUpdateRubro(dtoKey, rubro);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteRubro(string dtoKey)
			{
				this.HandleDeleteRubro(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Rubros returned per page.
			/// </summary>
			public int RubrosPerPageCount
			{
				get 
				{ 
					return this.rubrosPerPageCount; 
				}
				set 
				{
					if (value != this.rubrosPerPageCount)
					{
						this.rubrosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IRubroService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new RubroService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a rubro to the database and return the rubro with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="rubro">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected RubroDto Create(RubroDto rubro)
			{
				try
				{
					string dtoKey = this.Service.Add(rubro);
					this.UnitOfWork.SaveChanges();
					RubroDto addedRubro = this.Read(dtoKey);
					if (addedRubro == null)
					{
			...
			/// <summary>
			/// Returns the RubroDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected RubroDto Read(string dtoKey)
			{
				try
				{
					RubroDto rubro = this.Service.GetByKey(dtoKey);
					if (rubro == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, RubroDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the rubro with the specified dto key in the database. 
			...
			/// <param name="rubro">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, RubroDto rubro)
			{
				try
				{
					if (dtoKey != rubro.DtoKey && dtoKey != rubro.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
			...
			/// <summary>
			/// Deletes the rubro with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			/// <summary>
			/// Handles adding of rubro with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="rubro">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<RubroDto> HandleCreateRubro(RubroDto rubro, UriTemplate template)
			{
				RubroDto addedRubro = this.Create(rubro);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedRubro.DtoKey, template));
				ItemInfo<RubroDto> itemInfo = new ItemInfo<RubroDto>();
				itemInfo.Item = addedRubro;
				itemInfo.EditLink = GetItemLink(addedRubro.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of rubro with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected RubroDto HandleReadRubro(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Rubros with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<RubroDto> HandleReadRubros(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.RubrosPerPageCount > maxEntities)
					{
						this.RubrosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.RubrosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.RubrosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, RubroDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.RubrosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, RubroDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<RubroDto>> itemsInfo = new List<ItemInfo<RubroDto>>();
	
					foreach (KeyValuePair<string, RubroDto> item in items)
					{
						ItemInfo<RubroDto> itemInfo = new ItemInfo<RubroDto>();
					...
					return new ItemInfoList<RubroDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of rubro with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="rubro">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected RubroDto HandleUpdateRubro(string dtoKey, RubroDto rubro)
			{
				this.Update(dtoKey, rubro);
	
				return rubro;
			}
	
			/// <summary>
			/// Handles deleting of rubro with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteRubro(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// RubrosCollectionService finalizer.
			/// </summary>
			~RubrosCollectionService()
			{
