...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// CobrosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class CobrosCollectionService : ICobrosCollectionService
		{
			...
			#region ICobrosCollectionService Members
			...
			{
				return this.HandleReadCobro(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<CobroDto> ReadCobros()
			{
				return this.HandleReadCobros(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="cobro">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<CobroDto> CreateCobro(CobroDto cobro)
			{
				return this.HandleCreateCobro(cobro, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="cobro">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public CobroDto UpdateCobro(string dtoKey, CobroDto cobro)
			{
				return this.HandleUpdateCobro(dtoKey, cobro);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteCobro(string dtoKey)
			{
				this.HandleDeleteCobro(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Cobros returned per page.
			/// </summary>
			public int CobrosPerPageCount
			{
				get 
				{ 
					return this.cobrosPerPageCount; 
				}
				set 
				{
					if (value != this.cobrosPerPageCount)
					{
						this.cobrosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual ICobroService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new CobroService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a cobro to the database and return the cobro with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="cobro">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected CobroDto Create(CobroDto cobro)
			{
				try
				{
					string dtoKey = this.Service.Add(cobro);
					this.UnitOfWork.SaveChanges();
					CobroDto addedCobro = this.Read(dtoKey);
					if (addedCobro == null)
					{
			...
			/// <summary>
			/// Returns the CobroDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected CobroDto Read(string dtoKey)
			{
				try
				{
					CobroDto cobro = this.Service.GetByKey(dtoKey);
					if (cobro == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, CobroDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the cobro with the specified dto key in the database. 
			...
			/// <param name="cobro">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, CobroDto cobro)
			{
				try
				{
					if (dtoKey != cobro.DtoKey && dtoKey != cobro.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
			...
			/// <summary>
			/// Deletes the cobro with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			/// <summary>
			/// Handles adding of cobro with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="cobro">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<CobroDto> HandleCreateCobro(CobroDto cobro, UriTemplate template)
			{
				CobroDto addedCobro = this.Create(cobro);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedCobro.DtoKey, template));
				ItemInfo<CobroDto> itemInfo = new ItemInfo<CobroDto>();
				itemInfo.Item = addedCobro;
				itemInfo.EditLink = GetItemLink(addedCobro.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of cobro with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected CobroDto HandleReadCobro(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Cobros with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<CobroDto> HandleReadCobros(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.CobrosPerPageCount > maxEntities)
					{
						this.CobrosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.CobrosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.CobrosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, CobroDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.CobrosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, CobroDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<CobroDto>> itemsInfo = new List<ItemInfo<CobroDto>>();
	
					foreach (KeyValuePair<string, CobroDto> item in items)
					{
						ItemInfo<CobroDto> itemInfo = new ItemInfo<CobroDto>();
					...
					return new ItemInfoList<CobroDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of cobro with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="cobro">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected CobroDto HandleUpdateCobro(string dtoKey, CobroDto cobro)
			{
				this.Update(dtoKey, cobro);
	
				return cobro;
			}
	
			/// <summary>
			/// Handles deleting of cobro with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteCobro(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// CobrosCollectionService finalizer.
			/// </summary>
			~CobrosCollectionService()
			{
