...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// PredioPisosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class PredioPisosCollectionService : IPredioPisosCollectionService
		{
			...
			#region IPredioPisosCollectionService Members
			...
			{
				return this.HandleReadPredioPiso(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<PredioPisoDto> ReadPredioPisos()
			{
				return this.HandleReadPredioPisos(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="prediopiso">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<PredioPisoDto> CreatePredioPiso(PredioPisoDto prediopiso)
			{
				return this.HandleCreatePredioPiso(prediopiso, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="prediopiso">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public PredioPisoDto UpdatePredioPiso(string dtoKey, PredioPisoDto prediopiso)
			{
				return this.HandleUpdatePredioPiso(dtoKey, prediopiso);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeletePredioPiso(string dtoKey)
			{
				this.HandleDeletePredioPiso(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of PredioPisos returned per page.
			/// </summary>
			public int PredioPisosPerPageCount
			{
				get 
				{ 
					return this.prediopisosPerPageCount; 
				}
				set 
				{
					if (value != this.prediopisosPerPageCount)
					{
						this.prediopisosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IPredioPisoService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new PredioPisoService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a prediopiso to the database and return the prediopiso with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="prediopiso">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected PredioPisoDto Create(PredioPisoDto prediopiso)
			{
				try
				{
					string dtoKey = this.Service.Add(prediopiso);
					this.UnitOfWork.SaveChanges();
					PredioPisoDto addedPredioPiso = this.Read(dtoKey);
					if (addedPredioPiso == null)
					{
			...
			/// <summary>
			/// Returns the PredioPisoDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected PredioPisoDto Read(string dtoKey)
			{
				try
				{
					PredioPisoDto prediopiso = this.Service.GetByKey(dtoKey);
					if (prediopiso == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, PredioPisoDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the prediopiso with the specified dto key in the database. 
			...
			/// <param name="prediopiso">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, PredioPisoDto prediopiso)
			{
				try
				{
					if (dtoKey != prediopiso.DtoKey && dtoKey != prediopiso.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(prediopiso);
			...
			/// <summary>
			/// Deletes the prediopiso with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					PredioPisoDto prediopiso = Read(dtoKey);
					if (prediopiso == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(prediopiso);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Handles adding of prediopiso with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="prediopiso">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<PredioPisoDto> HandleCreatePredioPiso(PredioPisoDto prediopiso, UriTemplate template)
			{
				PredioPisoDto addedPredioPiso = this.Create(prediopiso);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedPredioPiso.DtoKey, template));
				ItemInfo<PredioPisoDto> itemInfo = new ItemInfo<PredioPisoDto>();
				itemInfo.Item = addedPredioPiso;
				itemInfo.EditLink = GetItemLink(addedPredioPiso.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of prediopiso with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected PredioPisoDto HandleReadPredioPiso(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of PredioPisos with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<PredioPisoDto> HandleReadPredioPisos(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.PredioPisosPerPageCount > maxEntities)
					{
						this.PredioPisosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.PredioPisosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.PredioPisosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, PredioPisoDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.PredioPisosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, PredioPisoDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<PredioPisoDto>> itemsInfo = new List<ItemInfo<PredioPisoDto>>();
	
					foreach (KeyValuePair<string, PredioPisoDto> item in items)
					{
						ItemInfo<PredioPisoDto> itemInfo = new ItemInfo<PredioPisoDto>();
					...
					return new ItemInfoList<PredioPisoDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of prediopiso with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="prediopiso">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected PredioPisoDto HandleUpdatePredioPiso(string dtoKey, PredioPisoDto prediopiso)
			{
				this.Update(dtoKey, prediopiso);
	
				return prediopiso;
			}
	
			/// <summary>
			/// Handles deleting of prediopiso with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeletePredioPiso(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// PredioPisosCollectionService finalizer.
			/// </summary>
			~PredioPisosCollectionService()
			{
