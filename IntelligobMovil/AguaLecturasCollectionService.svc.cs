...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// AguaLecturasCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class AguaLecturasCollectionService : IAguaLecturasCollectionService
		{
			...
			#region IAguaLecturasCollectionService Members
			...
			{
				return this.HandleReadAguaLectura(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<AguaLecturaDto> ReadAguaLecturas()
			{
				return this.HandleReadAguaLecturas(itemUriTemplate);
			}
			...
			/// <returns></returns>
			public ItemInfo<AguaLecturaDto> CreateAguaLectura(AguaLecturaDto agualectura)
			{
				return this.HandleCreateAguaLectura(agualectura, itemUriTemplate);
			}
			...
			/// <returns></returns>
			public AguaLecturaDto UpdateAguaLectura(string dtoKey, AguaLecturaDto agualectura)
			{
				return this.HandleUpdateAguaLectura(dtoKey, agualectura);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteAguaLectura(string dtoKey)
			{
				this.HandleDeleteAguaLectura(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of AguaLecturas returned per page.
			/// </summary>
			public int AguaLecturasPerPageCount
			{
				get 
				{ 
					return this.agualecturasPerPageCount; 
				}
				set 
				{
					if (value != this.agualecturasPerPageCount)
					{
						this.agualecturasPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IAguaLecturaService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new AguaLecturaService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a agualectura to the database and return the agualectura with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="agualectura">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected AguaLecturaDto Create(AguaLecturaDto agualectura)
			{
				try
				{
					string dtoKey = this.Service.Add(agualectura);
					this.UnitOfWork.SaveChanges();
					AguaLecturaDto addedAguaLectura = this.Read(dtoKey);
					if (addedAguaLectura == null)
					{
			...
			/// <summary>
			/// Returns the AguaLecturaDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected AguaLecturaDto Read(string dtoKey)
			{
				try
				{
					AguaLecturaDto agualectura = this.Service.GetByKey(dtoKey);
					if (agualectura == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, AguaLecturaDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the agualectura with the specified dto key in the database. 
			...
			/// <param name="agualectura">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, AguaLecturaDto agualectura)
			{
				try
				{
					if (dtoKey != agualectura.DtoKey && dtoKey != agualectura.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(agualectura);
			...
			/// <summary>
			/// Deletes the agualectura with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					AguaLecturaDto agualectura = Read(dtoKey);
					if (agualectura == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(agualectura);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Handles adding of agualectura with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="agualectura">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<AguaLecturaDto> HandleCreateAguaLectura(AguaLecturaDto agualectura, UriTemplate template)
			{
				AguaLecturaDto addedAguaLectura = this.Create(agualectura);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedAguaLectura.DtoKey, template));
				ItemInfo<AguaLecturaDto> itemInfo = new ItemInfo<AguaLecturaDto>();
				itemInfo.Item = addedAguaLectura;
				itemInfo.EditLink = GetItemLink(addedAguaLectura.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of agualectura with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected AguaLecturaDto HandleReadAguaLectura(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of AguaLecturas with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<AguaLecturaDto> HandleReadAguaLecturas(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.AguaLecturasPerPageCount > maxEntities)
					{
						this.AguaLecturasPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.AguaLecturasPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.AguaLecturasPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, AguaLecturaDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.AguaLecturasPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, AguaLecturaDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<AguaLecturaDto>> itemsInfo = new List<ItemInfo<AguaLecturaDto>>();
	
					foreach (KeyValuePair<string, AguaLecturaDto> item in items)
					{
						ItemInfo<AguaLecturaDto> itemInfo = new ItemInfo<AguaLecturaDto>();
					...
					return new ItemInfoList<AguaLecturaDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of agualectura with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="agualectura">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected AguaLecturaDto HandleUpdateAguaLectura(string dtoKey, AguaLecturaDto agualectura)
			{
				this.Update(dtoKey, agualectura);
	
				return agualectura;
			}
	
			/// <summary>
			/// Handles deleting of agualectura with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteAguaLectura(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// AguaLecturasCollectionService finalizer.
			/// </summary>
			~AguaLecturasCollectionService()
			{
