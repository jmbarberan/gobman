...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// AguaPotablesCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class AguaPotablesCollectionService : IAguaPotablesCollectionService
		{
			...
			#region IAguaPotablesCollectionService Members
			...
			{
				return this.HandleReadAguaPotable(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<AguaPotableDto> ReadAguaPotables()
			{
				return this.HandleReadAguaPotables(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="aguapotable">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<AguaPotableDto> CreateAguaPotable(AguaPotableDto aguapotable)
			{
				return this.HandleCreateAguaPotable(aguapotable, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="aguapotable">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public AguaPotableDto UpdateAguaPotable(string dtoKey, AguaPotableDto aguapotable)
			{
				return this.HandleUpdateAguaPotable(dtoKey, aguapotable);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteAguaPotable(string dtoKey)
			{
				this.HandleDeleteAguaPotable(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of AguaPotables returned per page.
			/// </summary>
			public int AguaPotablesPerPageCount
			{
				get 
				{ 
					return this.aguapotablesPerPageCount; 
				}
				set 
				{
					if (value != this.aguapotablesPerPageCount)
					{
						this.aguapotablesPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IAguaPotableService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new AguaPotableService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a aguapotable to the database and return the aguapotable with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="aguapotable">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected AguaPotableDto Create(AguaPotableDto aguapotable)
			{
				try
				{
					string dtoKey = this.Service.Add(aguapotable);
					this.UnitOfWork.SaveChanges();
					AguaPotableDto addedAguaPotable = this.Read(dtoKey);
					if (addedAguaPotable == null)
					{
			...
			/// <summary>
			/// Returns the AguaPotableDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected AguaPotableDto Read(string dtoKey)
			{
				try
				{
					AguaPotableDto aguapotable = this.Service.GetByKey(dtoKey);
					if (aguapotable == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, AguaPotableDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the aguapotable with the specified dto key in the database. 
			...
			/// <param name="aguapotable">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, AguaPotableDto aguapotable)
			{
				try
				{
					if (dtoKey != aguapotable.DtoKey && dtoKey != aguapotable.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(aguapotable);
			...
			/// <summary>
			/// Deletes the aguapotable with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					AguaPotableDto aguapotable = Read(dtoKey);
					if (aguapotable == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(aguapotable);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Handles adding of aguapotable with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="aguapotable">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<AguaPotableDto> HandleCreateAguaPotable(AguaPotableDto aguapotable, UriTemplate template)
			{
				AguaPotableDto addedAguaPotable = this.Create(aguapotable);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedAguaPotable.DtoKey, template));
				ItemInfo<AguaPotableDto> itemInfo = new ItemInfo<AguaPotableDto>();
				itemInfo.Item = addedAguaPotable;
				itemInfo.EditLink = GetItemLink(addedAguaPotable.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of aguapotable with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected AguaPotableDto HandleReadAguaPotable(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of AguaPotables with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<AguaPotableDto> HandleReadAguaPotables(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.AguaPotablesPerPageCount > maxEntities)
					{
						this.AguaPotablesPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.AguaPotablesPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.AguaPotablesPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, AguaPotableDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.AguaPotablesPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, AguaPotableDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<AguaPotableDto>> itemsInfo = new List<ItemInfo<AguaPotableDto>>();
	
					foreach (KeyValuePair<string, AguaPotableDto> item in items)
					{
						ItemInfo<AguaPotableDto> itemInfo = new ItemInfo<AguaPotableDto>();
					...
					return new ItemInfoList<AguaPotableDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of aguapotable with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="aguapotable">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected AguaPotableDto HandleUpdateAguaPotable(string dtoKey, AguaPotableDto aguapotable)
			{
				this.Update(dtoKey, aguapotable);
	
				return aguapotable;
			}
	
			/// <summary>
			/// Handles deleting of aguapotable with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteAguaPotable(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// AguaPotablesCollectionService finalizer.
			/// </summary>
			~AguaPotablesCollectionService()
			{
