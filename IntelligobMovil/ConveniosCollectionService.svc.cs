...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// ConveniosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class ConveniosCollectionService : IConveniosCollectionService
		{
			...
			#region IConveniosCollectionService Members
			...
			{
				return this.HandleReadConvenio(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<ConvenioDto> ReadConvenios()
			{
				return this.HandleReadConvenios(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="convenio">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<ConvenioDto> CreateConvenio(ConvenioDto convenio)
			{
				return this.HandleCreateConvenio(convenio, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="convenio">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public ConvenioDto UpdateConvenio(string dtoKey, ConvenioDto convenio)
			{
				return this.HandleUpdateConvenio(dtoKey, convenio);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteConvenio(string dtoKey)
			{
				this.HandleDeleteConvenio(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Convenios returned per page.
			/// </summary>
			public int ConveniosPerPageCount
			{
				get 
				{ 
					return this.conveniosPerPageCount; 
				}
				set 
				{
					if (value != this.conveniosPerPageCount)
					{
						this.conveniosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IConvenioService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new ConvenioService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a convenio to the database and return the convenio with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="convenio">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected ConvenioDto Create(ConvenioDto convenio)
			{
				try
				{
					string dtoKey = this.Service.Add(convenio);
					this.UnitOfWork.SaveChanges();
					ConvenioDto addedConvenio = this.Read(dtoKey);
					if (addedConvenio == null)
					{
			...
			/// <summary>
			/// Returns the ConvenioDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ConvenioDto Read(string dtoKey)
			{
				try
				{
					ConvenioDto convenio = this.Service.GetByKey(dtoKey);
					if (convenio == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, ConvenioDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the convenio with the specified dto key in the database. 
			...
			/// <param name="convenio">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, ConvenioDto convenio)
			{
				try
				{
					if (dtoKey != convenio.DtoKey && dtoKey != convenio.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
			...
			/// <summary>
			/// Deletes the convenio with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					ConvenioDto convenio = Read(dtoKey);
					if (convenio == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
			...
			/// <summary>
			/// Handles adding of convenio with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="convenio">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<ConvenioDto> HandleCreateConvenio(ConvenioDto convenio, UriTemplate template)
			{
				ConvenioDto addedConvenio = this.Create(convenio);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedConvenio.DtoKey, template));
				ItemInfo<ConvenioDto> itemInfo = new ItemInfo<ConvenioDto>();
				itemInfo.Item = addedConvenio;
				itemInfo.EditLink = GetItemLink(addedConvenio.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of convenio with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ConvenioDto HandleReadConvenio(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Convenios with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<ConvenioDto> HandleReadConvenios(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.ConveniosPerPageCount > maxEntities)
					{
						this.ConveniosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.ConveniosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.ConveniosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, ConvenioDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.ConveniosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, ConvenioDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<ConvenioDto>> itemsInfo = new List<ItemInfo<ConvenioDto>>();
	
					foreach (KeyValuePair<string, ConvenioDto> item in items)
					{
						ItemInfo<ConvenioDto> itemInfo = new ItemInfo<ConvenioDto>();
					...
					return new ItemInfoList<ConvenioDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of convenio with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="convenio">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected ConvenioDto HandleUpdateConvenio(string dtoKey, ConvenioDto convenio)
			{
				this.Update(dtoKey, convenio);
	
				return convenio;
			}
	
			/// <summary>
			/// Handles deleting of convenio with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteConvenio(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// ConveniosCollectionService finalizer.
			/// </summary>
			~ConveniosCollectionService()
			{
