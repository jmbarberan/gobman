...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// RepEmisionesSaldosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class RepEmisionesSaldosCollectionService : IRepEmisionesSaldosCollectionService
		{
			...
			#region IRepEmisionesSaldosCollectionService Members
			...
			{
				return this.HandleReadRepEmisionesSaldo(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<RepEmisionesSaldoDto> ReadRepEmisionesSaldos()
			{
				return this.HandleReadRepEmisionesSaldos(itemUriTemplate);
			}
			...
			/// <returns></returns>
			public ItemInfo<RepEmisionesSaldoDto> CreateRepEmisionesSaldo(RepEmisionesSaldoDto repemisionessaldo)
			{
				return this.HandleCreateRepEmisionesSaldo(repemisionessaldo, itemUriTemplate);
			}
			...
			/// <returns></returns>
			public RepEmisionesSaldoDto UpdateRepEmisionesSaldo(string dtoKey, RepEmisionesSaldoDto repemisionessaldo)
			{
				return this.HandleUpdateRepEmisionesSaldo(dtoKey, repemisionessaldo);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteRepEmisionesSaldo(string dtoKey)
			{
				this.HandleDeleteRepEmisionesSaldo(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of RepEmisionesSaldos returned per page.
			/// </summary>
			public int RepEmisionesSaldosPerPageCount
			{
				get 
				{ 
					return this.repemisionessaldosPerPageCount; 
				}
				set 
				{
					if (value != this.repemisionessaldosPerPageCount)
					{
						this.repemisionessaldosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IRepEmisionesSaldoService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new RepEmisionesSaldoService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a repemisionessaldo to the database and return the repemisionessaldo with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="repemisionessaldo">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected RepEmisionesSaldoDto Create(RepEmisionesSaldoDto repemisionessaldo)
			{
				try
				{
					string dtoKey = this.Service.Add(repemisionessaldo);
					this.UnitOfWork.SaveChanges();
					RepEmisionesSaldoDto addedRepEmisionesSaldo = this.Read(dtoKey);
					if (addedRepEmisionesSaldo == null)
					{
			...
			/// <summary>
			/// Returns the RepEmisionesSaldoDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected RepEmisionesSaldoDto Read(string dtoKey)
			{
				try
				{
					RepEmisionesSaldoDto repemisionessaldo = this.Service.GetByKey(dtoKey);
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, RepEmisionesSaldoDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the repemisionessaldo with the specified dto key in the database. 
			...
			/// <param name="repemisionessaldo">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, RepEmisionesSaldoDto repemisionessaldo)
			{
				try
				{
					if (dtoKey != repemisionessaldo.DtoKey && dtoKey != repemisionessaldo.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(repemisionessaldo);
			...
			/// <summary>
			/// Deletes the repemisionessaldo with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					RepEmisionesSaldoDto repemisionessaldo = Read(dtoKey);
					if (repemisionessaldo == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(repemisionessaldo);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Returns the repemisionessaldo count.
			/// </summary>
			/// <returns></returns>
			protected int Count()
			{
			...
			/// <summary>
			/// Handles adding of repemisionessaldo with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="repemisionessaldo">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<RepEmisionesSaldoDto> HandleCreateRepEmisionesSaldo(RepEmisionesSaldoDto repemisionessaldo, UriTemplate template)
			{
				RepEmisionesSaldoDto addedRepEmisionesSaldo = this.Create(repemisionessaldo);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedRepEmisionesSaldo.DtoKey, template));
				ItemInfo<RepEmisionesSaldoDto> itemInfo = new ItemInfo<RepEmisionesSaldoDto>();
				itemInfo.Item = addedRepEmisionesSaldo;
				itemInfo.EditLink = GetItemLink(addedRepEmisionesSaldo.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of repemisionessaldo with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected RepEmisionesSaldoDto HandleReadRepEmisionesSaldo(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of RepEmisionesSaldos with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<RepEmisionesSaldoDto> HandleReadRepEmisionesSaldos(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.RepEmisionesSaldosPerPageCount > maxEntities)
					{
						this.RepEmisionesSaldosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.RepEmisionesSaldosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.RepEmisionesSaldosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, RepEmisionesSaldoDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.RepEmisionesSaldosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, RepEmisionesSaldoDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<RepEmisionesSaldoDto>> itemsInfo = new List<ItemInfo<RepEmisionesSaldoDto>>();
	
					foreach (KeyValuePair<string, RepEmisionesSaldoDto> item in items)
					{
						ItemInfo<RepEmisionesSaldoDto> itemInfo = new ItemInfo<RepEmisionesSaldoDto>();
					...
					return new ItemInfoList<RepEmisionesSaldoDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of repemisionessaldo with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="repemisionessaldo">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected RepEmisionesSaldoDto HandleUpdateRepEmisionesSaldo(string dtoKey, RepEmisionesSaldoDto repemisionessaldo)
			{
				this.Update(dtoKey, repemisionessaldo);
	
				return repemisionessaldo;
			}
	
			/// <summary>
			/// Handles deleting of repemisionessaldo with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteRepEmisionesSaldo(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// RepEmisionesSaldosCollectionService finalizer.
			/// </summary>
			~RepEmisionesSaldosCollectionService()
			{
