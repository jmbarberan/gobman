...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// ServiciosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class ServiciosCollectionService : IServiciosCollectionService
		{
			...
			/// </summary>
			private int serviciosPerPageCount = 5;
			...
			#region IServiciosCollectionService Members
			...
			{
				return this.HandleReadServicio(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<ServicioDto> ReadServicios()
			{
				return this.HandleReadServicios(itemUriTemplate);
			}
			...
			/// <returns></returns>
			public ItemInfo<ServicioDto> CreateServicio(ServicioDto servicio)
			{
				return this.HandleCreateServicio(servicio, itemUriTemplate);
			}
			...
			/// <returns></returns>
			public ServicioDto UpdateServicio(string dtoKey, ServicioDto servicio)
			{
				return this.HandleUpdateServicio(dtoKey, servicio);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteServicio(string dtoKey)
			{
				this.HandleDeleteServicio(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Servicios returned per page.
			/// </summary>
			public int ServiciosPerPageCount
			{
				get 
				{ 
					return this.serviciosPerPageCount; 
				}
				set 
				{
					if (value != this.serviciosPerPageCount)
					{
						this.serviciosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IServicioService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new ServicioService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a servicio to the database and return the servicio with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="servicio">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected ServicioDto Create(ServicioDto servicio)
			{
				try
				{
					string dtoKey = this.Service.Add(servicio);
					this.UnitOfWork.SaveChanges();
					ServicioDto addedServicio = this.Read(dtoKey);
			...
			/// <summary>
			/// Returns the ServicioDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ServicioDto Read(string dtoKey)
			{
				try
				{
					ServicioDto servicio = this.Service.GetByKey(dtoKey);
					if (servicio == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, ServicioDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the servicio with the specified dto key in the database. 
			...
			/// <param name="servicio">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, ServicioDto servicio)
			{
				try
				{
					if (dtoKey != servicio.DtoKey && dtoKey != servicio.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
			...
			/// <summary>
			/// Deletes the servicio with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					ServicioDto servicio = Read(dtoKey);
					if (servicio == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
			...
			/// <summary>
			/// Handles adding of servicio with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="servicio">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<ServicioDto> HandleCreateServicio(ServicioDto servicio, UriTemplate template)
			{
				ServicioDto addedServicio = this.Create(servicio);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedServicio.DtoKey, template));
				ItemInfo<ServicioDto> itemInfo = new ItemInfo<ServicioDto>();
				itemInfo.Item = addedServicio;
				itemInfo.EditLink = GetItemLink(addedServicio.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of servicio with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected ServicioDto HandleReadServicio(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Servicios with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<ServicioDto> HandleReadServicios(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.ServiciosPerPageCount > maxEntities)
					{
						this.ServiciosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.ServiciosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.ServiciosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, ServicioDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.ServiciosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, ServicioDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<ServicioDto>> itemsInfo = new List<ItemInfo<ServicioDto>>();
	
					foreach (KeyValuePair<string, ServicioDto> item in items)
					{
						ItemInfo<ServicioDto> itemInfo = new ItemInfo<ServicioDto>();
					...
					return new ItemInfoList<ServicioDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of servicio with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="servicio">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected ServicioDto HandleUpdateServicio(string dtoKey, ServicioDto servicio)
			{
				this.Update(dtoKey, servicio);
	
				return servicio;
			}
	
			/// <summary>
			/// Handles deleting of servicio with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteServicio(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// ServiciosCollectionService finalizer.
			/// </summary>
			~ServiciosCollectionService()
			{
