...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// CobroTransaccionsCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class CobroTransaccionsCollectionService : ICobroTransaccionsCollectionService
		{
			...
			#region ICobroTransaccionsCollectionService Members
			...
			{
				return this.HandleReadCobroTransaccion(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<CobroTransaccionDto> ReadCobroTransaccions()
			{
				return this.HandleReadCobroTransaccions(itemUriTemplate);
			}
			...
			/// <returns></returns>
			public ItemInfo<CobroTransaccionDto> CreateCobroTransaccion(CobroTransaccionDto cobrotransaccion)
			{
				return this.HandleCreateCobroTransaccion(cobrotransaccion, itemUriTemplate);
			}
			...
			/// <returns></returns>
			public CobroTransaccionDto UpdateCobroTransaccion(string dtoKey, CobroTransaccionDto cobrotransaccion)
			{
				return this.HandleUpdateCobroTransaccion(dtoKey, cobrotransaccion);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteCobroTransaccion(string dtoKey)
			{
				this.HandleDeleteCobroTransaccion(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of CobroTransaccions returned per page.
			/// </summary>
			public int CobroTransaccionsPerPageCount
			{
				get 
				{ 
					return this.cobrotransaccionsPerPageCount; 
				}
				set 
				{
					if (value != this.cobrotransaccionsPerPageCount)
					{
						this.cobrotransaccionsPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual ICobroTransaccionService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new CobroTransaccionService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a cobrotransaccion to the database and return the cobrotransaccion with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="cobrotransaccion">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected CobroTransaccionDto Create(CobroTransaccionDto cobrotransaccion)
			{
				try
				{
					string dtoKey = this.Service.Add(cobrotransaccion);
					this.UnitOfWork.SaveChanges();
					CobroTransaccionDto addedCobroTransaccion = this.Read(dtoKey);
					if (addedCobroTransaccion == null)
					{
			...
			/// <summary>
			/// Returns the CobroTransaccionDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected CobroTransaccionDto Read(string dtoKey)
			{
				try
				{
					CobroTransaccionDto cobrotransaccion = this.Service.GetByKey(dtoKey);
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, CobroTransaccionDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the cobrotransaccion with the specified dto key in the database. 
			...
			/// <param name="cobrotransaccion">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, CobroTransaccionDto cobrotransaccion)
			{
				try
				{
					if (dtoKey != cobrotransaccion.DtoKey && dtoKey != cobrotransaccion.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(cobrotransaccion);
			...
			/// <summary>
			/// Deletes the cobrotransaccion with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					CobroTransaccionDto cobrotransaccion = Read(dtoKey);
					if (cobrotransaccion == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(cobrotransaccion);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Returns the cobrotransaccion count.
			/// </summary>
			/// <returns></returns>
			protected int Count()
			{
			...
			/// <summary>
			/// Handles adding of cobrotransaccion with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="cobrotransaccion">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<CobroTransaccionDto> HandleCreateCobroTransaccion(CobroTransaccionDto cobrotransaccion, UriTemplate template)
			{
				CobroTransaccionDto addedCobroTransaccion = this.Create(cobrotransaccion);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedCobroTransaccion.DtoKey, template));
				ItemInfo<CobroTransaccionDto> itemInfo = new ItemInfo<CobroTransaccionDto>();
				itemInfo.Item = addedCobroTransaccion;
				itemInfo.EditLink = GetItemLink(addedCobroTransaccion.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of cobrotransaccion with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected CobroTransaccionDto HandleReadCobroTransaccion(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of CobroTransaccions with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<CobroTransaccionDto> HandleReadCobroTransaccions(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.CobroTransaccionsPerPageCount > maxEntities)
					{
						this.CobroTransaccionsPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.CobroTransaccionsPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.CobroTransaccionsPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, CobroTransaccionDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.CobroTransaccionsPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, CobroTransaccionDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<CobroTransaccionDto>> itemsInfo = new List<ItemInfo<CobroTransaccionDto>>();
	
					foreach (KeyValuePair<string, CobroTransaccionDto> item in items)
					{
						ItemInfo<CobroTransaccionDto> itemInfo = new ItemInfo<CobroTransaccionDto>();
					...
					return new ItemInfoList<CobroTransaccionDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of cobrotransaccion with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="cobrotransaccion">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected CobroTransaccionDto HandleUpdateCobroTransaccion(string dtoKey, CobroTransaccionDto cobrotransaccion)
			{
				this.Update(dtoKey, cobrotransaccion);
	
				return cobrotransaccion;
			}
	
			/// <summary>
			/// Handles deleting of cobrotransaccion with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteCobroTransaccion(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// CobroTransaccionsCollectionService finalizer.
			/// </summary>
			~CobroTransaccionsCollectionService()
			{
