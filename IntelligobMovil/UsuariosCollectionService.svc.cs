...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// UsuariosCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class UsuariosCollectionService : IUsuariosCollectionService
		{
			...
			/// </summary>
			private int usuariosPerPageCount = 5;
			...
			#region IUsuariosCollectionService Members
			...
			{
				return this.HandleReadUsuario(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<UsuarioDto> ReadUsuarios()
			{
				return this.HandleReadUsuarios(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="usuario">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<UsuarioDto> CreateUsuario(UsuarioDto usuario)
			{
				return this.HandleCreateUsuario(usuario, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="usuario">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public UsuarioDto UpdateUsuario(string dtoKey, UsuarioDto usuario)
			{
				return this.HandleUpdateUsuario(dtoKey, usuario);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteUsuario(string dtoKey)
			{
				this.HandleDeleteUsuario(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Usuarios returned per page.
			/// </summary>
			public int UsuariosPerPageCount
			{
				get 
				{ 
					return this.usuariosPerPageCount; 
				}
				set 
				{
					if (value != this.usuariosPerPageCount)
					{
						this.usuariosPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IUsuarioService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new UsuarioService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a usuario to the database and return the usuario with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="usuario">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected UsuarioDto Create(UsuarioDto usuario)
			{
				try
				{
					string dtoKey = this.Service.Add(usuario);
					this.UnitOfWork.SaveChanges();
					UsuarioDto addedUsuario = this.Read(dtoKey);
			...
			/// <summary>
			/// Returns the UsuarioDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected UsuarioDto Read(string dtoKey)
			{
				try
				{
					UsuarioDto usuario = this.Service.GetByKey(dtoKey);
					if (usuario == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, UsuarioDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the usuario with the specified dto key in the database. 
			...
			/// <param name="usuario">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, UsuarioDto usuario)
			{
				try
				{
					if (dtoKey != usuario.DtoKey && dtoKey != usuario.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
			...
			/// <summary>
			/// Deletes the usuario with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					UsuarioDto usuario = Read(dtoKey);
					if (usuario == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
			...
			/// <summary>
			/// Handles adding of usuario with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="usuario">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<UsuarioDto> HandleCreateUsuario(UsuarioDto usuario, UriTemplate template)
			{
				UsuarioDto addedUsuario = this.Create(usuario);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedUsuario.DtoKey, template));
				ItemInfo<UsuarioDto> itemInfo = new ItemInfo<UsuarioDto>();
				itemInfo.Item = addedUsuario;
				itemInfo.EditLink = GetItemLink(addedUsuario.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of usuario with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected UsuarioDto HandleReadUsuario(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Usuarios with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<UsuarioDto> HandleReadUsuarios(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.UsuariosPerPageCount > maxEntities)
					{
						this.UsuariosPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.UsuariosPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.UsuariosPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, UsuarioDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.UsuariosPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, UsuarioDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<UsuarioDto>> itemsInfo = new List<ItemInfo<UsuarioDto>>();
	
					foreach (KeyValuePair<string, UsuarioDto> item in items)
					{
						ItemInfo<UsuarioDto> itemInfo = new ItemInfo<UsuarioDto>();
					...
					return new ItemInfoList<UsuarioDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of usuario with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="usuario">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected UsuarioDto HandleUpdateUsuario(string dtoKey, UsuarioDto usuario)
			{
				this.Update(dtoKey, usuario);
	
				return usuario;
			}
	
			/// <summary>
			/// Handles deleting of usuario with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteUsuario(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// UsuariosCollectionService finalizer.
			/// </summary>
			~UsuariosCollectionService()
			{
