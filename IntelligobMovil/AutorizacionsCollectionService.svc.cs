...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// AutorizacionsCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class AutorizacionsCollectionService : IAutorizacionsCollectionService
		{
			...
			#region IAutorizacionsCollectionService Members
			...
			{
				return this.HandleReadAutorizacion(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<AutorizacionDto> ReadAutorizacions()
			{
				return this.HandleReadAutorizacions(itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="autorizacion">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			public ItemInfo<AutorizacionDto> CreateAutorizacion(AutorizacionDto autorizacion)
			{
				return this.HandleCreateAutorizacion(autorizacion, itemUriTemplate);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="autorizacion">The dto that holds the data to be updated.</param>	
			/// <returns></returns>
			public AutorizacionDto UpdateAutorizacion(string dtoKey, AutorizacionDto autorizacion)
			{
				return this.HandleUpdateAutorizacion(dtoKey, autorizacion);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteAutorizacion(string dtoKey)
			{
				this.HandleDeleteAutorizacion(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Autorizacions returned per page.
			/// </summary>
			public int AutorizacionsPerPageCount
			{
				get 
				{ 
					return this.autorizacionsPerPageCount; 
				}
				set 
				{
					if (value != this.autorizacionsPerPageCount)
					{
						this.autorizacionsPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual IAutorizacionService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new AutorizacionService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a autorizacion to the database and return the autorizacion with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="autorizacion">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected AutorizacionDto Create(AutorizacionDto autorizacion)
			{
				try
				{
					string dtoKey = this.Service.Add(autorizacion);
					this.UnitOfWork.SaveChanges();
					AutorizacionDto addedAutorizacion = this.Read(dtoKey);
					if (addedAutorizacion == null)
					{
			...
			/// <summary>
			/// Returns the AutorizacionDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected AutorizacionDto Read(string dtoKey)
			{
				try
				{
					AutorizacionDto autorizacion = this.Service.GetByKey(dtoKey);
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, AutorizacionDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the autorizacion with the specified dto key in the database. 
			...
			/// <param name="autorizacion">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, AutorizacionDto autorizacion)
			{
				try
				{
					if (dtoKey != autorizacion.DtoKey && dtoKey != autorizacion.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
					...
					this.Service.Update(autorizacion);
			...
			/// <summary>
			/// Deletes the autorizacion with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					AutorizacionDto autorizacion = Read(dtoKey);
					if (autorizacion == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
					...
					this.Service.Delete(autorizacion);
					this.UnitOfWork.SaveChanges();
			...
			/// <summary>
			/// Returns the autorizacion count.
			/// </summary>
			/// <returns></returns>
			protected int Count()
			{
			...
			/// <summary>
			/// Handles adding of autorizacion with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="autorizacion">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<AutorizacionDto> HandleCreateAutorizacion(AutorizacionDto autorizacion, UriTemplate template)
			{
				AutorizacionDto addedAutorizacion = this.Create(autorizacion);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedAutorizacion.DtoKey, template));
				ItemInfo<AutorizacionDto> itemInfo = new ItemInfo<AutorizacionDto>();
				itemInfo.Item = addedAutorizacion;
				itemInfo.EditLink = GetItemLink(addedAutorizacion.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of autorizacion with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected AutorizacionDto HandleReadAutorizacion(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Autorizacions with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<AutorizacionDto> HandleReadAutorizacions(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.AutorizacionsPerPageCount > maxEntities)
					{
						this.AutorizacionsPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.AutorizacionsPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.AutorizacionsPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, AutorizacionDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.AutorizacionsPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, AutorizacionDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<AutorizacionDto>> itemsInfo = new List<ItemInfo<AutorizacionDto>>();
	
					foreach (KeyValuePair<string, AutorizacionDto> item in items)
					{
						ItemInfo<AutorizacionDto> itemInfo = new ItemInfo<AutorizacionDto>();
					...
					return new ItemInfoList<AutorizacionDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of autorizacion with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="autorizacion">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected AutorizacionDto HandleUpdateAutorizacion(string dtoKey, AutorizacionDto autorizacion)
			{
				this.Update(dtoKey, autorizacion);
	
				return autorizacion;
			}
	
			/// <summary>
			/// Handles deleting of autorizacion with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteAutorizacion(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// AutorizacionsCollectionService finalizer.
			/// </summary>
			~AutorizacionsCollectionService()
			{
