...
namespace IntelligobMovil
{
	...
	using System.Runtime.Serialization;
		...
		/// <summary>
		/// SoportesCollectionService service class handler.
		/// </summary> 
		[ServiceBehavior(InstanceContextMode = InstanceContextMode.PerCall)]
		public partial class SoportesCollectionService : ISoportesCollectionService
		{
			...
			/// </summary>
			private int soportesPerPageCount = 5;
			...
			#region ISoportesCollectionService Members
			...
			{
				return this.HandleReadSoporte(dtoKey);
			}
			...
			/// </summary>
			/// <returns></returns>
			public ItemInfoList<SoporteDto> ReadSoportes()
			{
				return this.HandleReadSoportes(itemUriTemplate);
			}
			...
			/// <returns></returns>
			public ItemInfo<SoporteDto> CreateSoporte(SoporteDto soporte)
			{
				return this.HandleCreateSoporte(soporte, itemUriTemplate);
			}
			...
			/// <returns></returns>
			public SoporteDto UpdateSoporte(string dtoKey, SoporteDto soporte)
			{
				return this.HandleUpdateSoporte(dtoKey, soporte);
			}
			...
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			public void DeleteSoporte(string dtoKey)
			{
				this.HandleDeleteSoporte(dtoKey);
			}
			...
			/// <summary>
			/// Gets or sets the number of Soportes returned per page.
			/// </summary>
			public int SoportesPerPageCount
			{
				get 
				{ 
					return this.soportesPerPageCount; 
				}
				set 
				{
					if (value != this.soportesPerPageCount)
					{
						this.soportesPerPageCount = value;
					}
				}
			}
			...
			/// </summary>
			protected virtual ISoporteService Service
			{
				get
				{
					if (this.service == null)
					{
						...
						this.service = new SoporteService(assembler, repository);
					}
					return this.service;
				}
			}
			...
			/// <summary>
			/// Adds a soporte to the database and return the soporte with updated dto key. Returns null if adding the item failed.
			/// A null return value will result in a response status code of InternalServerError (500).
			/// </summary>
			/// <param name="soporte">The dto that holds the data to be inserted.</param>
			/// <returns></returns>
			protected SoporteDto Create(SoporteDto soporte)
			{
				try
				{
					string dtoKey = this.Service.Add(soporte);
					this.UnitOfWork.SaveChanges();
					SoporteDto addedSoporte = this.Read(dtoKey);
					if (addedSoporte == null)
					{
			...
			/// <summary>
			/// Returns the SoporteDto with the given dto key. 
			...
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected SoporteDto Read(string dtoKey)
			{
				try
				{
					SoporteDto soporte = this.Service.GetByKey(dtoKey);
					if (soporte == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
			...
			/// <returns></returns>
			protected IEnumerable<KeyValuePair<string, SoporteDto>> Read(int startRowIndex, int maximumRows)
			{
			...
			/// <summary>
			/// Updates the soporte with the specified dto key in the database. 
			...
			/// <param name="soporte">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected void Update(string dtoKey, SoporteDto soporte)
			{
				try
				{
					if (dtoKey != soporte.DtoKey && dtoKey != soporte.Id.ToString(string.Empty, CultureInfo.InvariantCulture))
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.BadRequest;
			...
			/// <summary>
			/// Deletes the soporte with the specified dto key from the database.
			/// A response status code of NotFound (404) will be the result if the item does not exist.
			/// A response status code of InternalServerError (500) will be the result if an error occur.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			...
			protected bool Delete(string dtoKey)
			{
				try
				{
					SoporteDto soporte = Read(dtoKey);
					if (soporte == null)
					{
						OutgoingWebResponseContext response = WebOperationContext.Current.OutgoingResponse;
						response.StatusCode = HttpStatusCode.NotFound;
			...
			/// <summary>
			/// Handles adding of soporte with a specified URI template and return a wrapped item info object.
			/// </summary>
			/// <param name="soporte">The dto that holds the data to be added.</param>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfo<SoporteDto> HandleCreateSoporte(SoporteDto soporte, UriTemplate template)
			{
				SoporteDto addedSoporte = this.Create(soporte);
	
				WebOperationContext.Current.OutgoingResponse.SetStatusAsCreated(GetItemLink(addedSoporte.DtoKey, template));
				ItemInfo<SoporteDto> itemInfo = new ItemInfo<SoporteDto>();
				itemInfo.Item = addedSoporte;
				itemInfo.EditLink = GetItemLink(addedSoporte.DtoKey, template);
	
				return itemInfo;
			}
	
			/// <summary>
			/// Handles reading of soporte with a specified dto key and return a dto.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <returns></returns>
			protected SoporteDto HandleReadSoporte(string dtoKey)
			{
				return this.Read(dtoKey);
			}
	
			/// <summary>
			/// Handles reading of Soportes with a specified URI template and return a wrapped item info object list.
			/// </summary>
			/// <param name="template">The URI template to be used.</param>
			/// <returns></returns>
			protected ItemInfoList<SoporteDto> HandleReadSoportes(UriTemplate template)
			{
				int startIndex = 0;
				...
				if (pageQueryParam != null)
				{
					...
					if (this.SoportesPerPageCount > maxEntities)
					{
						this.SoportesPerPageCount = maxEntities;
					}
	
					bool hasMoreEntries = pageNumber * this.SoportesPerPageCount < maxEntities;
					...
					startIndex = pageNumber * this.SoportesPerPageCount;
				}
	
				IEnumerable<KeyValuePair<string, SoporteDto>> items =
					this.Read(
						startIndex,
						(pageQueryParam != null) ? this.SoportesPerPageCount : maxEntities);
	
				if (items == null)
				{
					items = new List<KeyValuePair<string, SoporteDto>>();
	
					return null;
				}
				else
				{
					List<ItemInfo<SoporteDto>> itemsInfo = new List<ItemInfo<SoporteDto>>();
	
					foreach (KeyValuePair<string, SoporteDto> item in items)
					{
						ItemInfo<SoporteDto> itemInfo = new ItemInfo<SoporteDto>();
					...
					return new ItemInfoList<SoporteDto>(itemsInfo, nextPageLink, previousPageLink);
				}
			}
	
			/// <summary>
			/// Handles updating of soporte with a specified dto key and return a the same dto with updated dto key property.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			/// <param name="soporte">The dto that holds the data to be updated.</param>
			/// <returns></returns>
			protected SoporteDto HandleUpdateSoporte(string dtoKey, SoporteDto soporte)
			{
				this.Update(dtoKey, soporte);
	
				return soporte;
			}
	
			/// <summary>
			/// Handles deleting of soporte with a specified dto key.
			/// </summary>
			/// <param name="dtoKey">The specified dto key value.</param>
			protected void HandleDeleteSoporte(string dtoKey)
			{
				this.Delete(dtoKey);
			}
			...
			/// <summary>
			/// SoportesCollectionService finalizer.
			/// </summary>
			~SoportesCollectionService()
			{
